...
1. Before a release, run the following babel script to refresh the 'expected' screenshots (by clicking C-c twice). If the test ~tool/demo/run_all_examples.sh~ succeeds, it is guarenteed the 'expected' screenshots are same as those produced by the code in ~example1/lib/main.dart~, which is used to generate code in this README file.
