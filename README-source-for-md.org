...
1. Run the following babel script which refreshes the 'expected' screenshots and also creates a 150px wide version. Do so by clicking C-c twice in the begin_src section. If the test ~tool/demo/run_all_examples.sh~ succeeds, it is guarenteed the 'expected' screenshots are same as those produced by the code in ~example1/lib/main.dart~. The ~example1/lib/main.dart~ is also used to generate images gallery with links to code in this README file on top.
