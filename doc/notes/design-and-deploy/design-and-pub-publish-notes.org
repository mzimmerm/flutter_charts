...
** IN-PROGRESS-NOW Replace ChartData and add DataPoint, more representing what people need, and adding logarithic scale
...
***** IN-PROGRESS-NOW Bug fix: Ex40 which is manual scale is now using logarithmic scale. Fix it.
...
***** IN-PROGRESS-NOW Steps to introduce logarithmic scale
 

- [ ] Add data test if scale is logarithmic: only allowed if yValues are numeric and >= 0, otherwise exception, method: checkDomainAndRangeValidFor(yTransform)  
- [ ] Add comments:
  - [ ] LabelInfo.dataValue should ONLY be used for labels, never for data or coordinates
  - [ ] *DISPLAYED DATA INSIDE the CHART ARE TRANSFORMED, DISPLAYED LABELS SHOW RAW DATA VALUES*
  - [ ] On  axisYMin than it is really axisYBottom (which is on the bottom of the chart, but has the max value). Similar to Max
- [ ] Check Vertical Bar log based: Is this a PROBLEM?: IN STACKED CHART, WE FIRST PASS LOG(Y), THEN ADD LOG(YS) DURING STACKING. THAT IS WRONG. THE STACKABLE COLUMNS MUST FIRS ACCEPT Y, THEN ADD DURING STACKING, THEN DO LOG(ADDED).


**** TODO TODO: Maybe use term RawDataRows, etc for Raw data.

- Anywhere, there is only dataRows and dataYs which represent transformed
...
**** TODO TODO: Replace labelInfo.dataValue with getter OR MAYBE REPLACE labelInfo.transformedDataValue with getter?
**** TODO TODO: Replace labelInfo.
