...
- [X] Add PaddingLayouter that can be used around single child to pad with space - no painting needed
  ...
  - [X] Test work
- [ ] Rename PaddingLayouter to Padding
- [ ] Add Aligner, a Layouter that centers single child - no painting needed
  - [ ] Extends Positioning layouter
  - [ ] Constructor is one-child, plus 2 optional arguments childWidthMultiple and childHeightMultiple + Alignment new class that defines Alignment.topRight, Alignment.topLeft, Alignment.bottomLeft, Alignment.bottomLeft, Alignment.Center 
  - [ ] If multiple along axis not set
    - [ ] pass self constraint to child, default
    - [ ] Child position is set in the middle of self constraint (may overflow of course). This uses the 
    - [ ] self layoutSize is set to self constraint size
  - [ ] If multiple along axis is set
    - [ ] pass self constraint to child, default
    - [ ] child position is set in the middle of to width from child's layuoutSize multiplied by Multiple - returned Rect 
    - [ ] offset child as default
    - [ ] self layout size is child layoutSize multiplied by multiple in the appropriate direction
  - [ ] Use AlignerLayouter in Legend and LegendItemContainer to make sure they are usable.
  - [ ] Test work
