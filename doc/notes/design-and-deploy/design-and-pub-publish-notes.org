...
*** IN-PROGRESS LayoutExpansion: more study and cleanup
...
- [ ] LayoutExpansion is now passed to layout as it functions basically as Constraint!!
-



*** ~HUGE Rename LayoutExpansion to Constraint~ !!!
...
***** Change code to pickup 
**** TODO Edit all calls to layout and return it

**** TODO ~Add constraints and layouts ~

                                     
- [ ] Add constraint.dart -
  - Constraint{ _maxSize = Size.infinity; Constraint({maxSize:}); Constraint.infinity: redirect to Constraint(maxSize: Size.infinity};},
- [ ] In existing code, Pass Constraint to each Container.layout({required Constraint constraint})
- [ ] Add Constraint field to Container, from constructor.
...
- [ ] add member size
- [ ] keep member offset
- [ ] implement default method layout(Constraint) - see [[Layouter general function]]
- [ ] add abstract method calculateAndSetActualChildrenSizesAndOffsetsInSelf() # Calculates children's positions in self based on what the layout actually is
- [ ] applyParentOffset removed (BUT KEEP AT FIRST) - replaced by calculateAndSetActualChildrenSizesAndOffsetsInSelf
...
- Generally, Containers lifecycle should be as follows.
  ...
  - Layout
    - called by root.layout(Constraint)
    - side effects
...
**** TODO Move some code to calculateAndSetActualChildrenSizesAndOffsetsInSelf
...
#+end_example
shoudl be moved to a XContainerManualLayout(new layout class).calculateAndSetActualChildrenSizesAndOffsetsInSelf
...
*** LATER ~LayoutExpansion~ - merge it somehow with ~Constraint~!!!
...
1. *Constraint is passed down* to layout call.                                               parent context: ~Size childPreferredSize = child.layout(constraintDefinedByParent)~  <= side effect: child.size is set (=childPreferredSize) 
...
4. Parent decides, based on it's layout algorithm,
   the size and position to actually give to each child :                                    parent context: calculateAndSetActualChildrenSizesAndOffsetsInSelf() <= size effect: child.size [overrides previous value] and child.offset (within parent) [new value] is set on each child. this.size and this.offset is set on me (parent)
6. Step 4 includes *Parent sets actual size and position down on child*.


#+BEGIN_SRC python
  def layout(Size constraintFromParent):     # 1. constraints go down (as this is called in parent context)
    if this is leaf:
      this.size = calculate own size (may be e.g. size of rectangle if this is a rectangle) if not withit Constraint, exception???
      return this.size                # 2. sizes go up
    else:
      for child in children:
        # All children are offered the full size of parent. If greedy, that is a problem 
        Size size = child.layout(constraintFromParent) # ignore return, we have it in side-effect
      calculateAndSetActualChildrenSizesAndOffsetsInSelf()
      return this.size ????


  # For example, for flow layout, each child is given the size it previously asked, and moved by predecessors sizes.
  # !!!!!!!!!!!!!!! this replaces applyParentOffset
  def calculateAndSetActualChildrenSizesAndOffsetsInSelf():
    Offset currentOffset = 0
    Size mySize = 0
    for child in children:
      child.size = child.size         # Size goes down - no change in one pass layout
      child.offset += currentOffset   # 3. Actual position goes down
      currentOffset += new Offset(child.size.width, 0) # just shift next child some more to the right.
      mySize = currentOffset + child.size
    this.size = mySize
#+END_SRC   
