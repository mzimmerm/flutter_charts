...
** IN-PROGRESS-NOW 11. ~CURRENT WORK: RENAME TO OUTER~
...
- [ ] Look for OWNER, consider if it needs rename to outer. Usually, this should be accompanied with a 'make' method, which should be renamed to 'makeInner'. FINISH THIS. LOOK FOR OWNER, BUT NOT IN unused.dart


problem: The affmapToPixels is only called in CL code. Need to fix this first!!!

NO: This is vs affmapValuesToPixels!!! compare them
...
** LATER 12. ~CURRENT WORK: ADD LINE CHART~
