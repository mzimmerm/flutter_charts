...
- [ ] Add ChartOptions.DataContainer.yTransform Function double -> double for now (yTransform applies to both DataContainer and YContainer) 
- [-] later implemented by ovewriting DataSetsMixin.yTransform (move this to later part)
...
- [ ] yTransform can always be applied as follows:
  - [ ] applied to yValues before creating DataPoints, PointsColumns, etc
  - [ ] applied to yValues Interval before range is created and scaled to displayScale
  - [ ] NOT applied in labelformatter - original unscaled values are used there
