...
In TableCellDefiner, add isOverflown (hmm, maybe separate width, height

In TableDefiner, add two 1D Lists:
rowHeights, columnWidths

 Add layout methods around post, overriding the method that creates the rectangles. Instead, this section will create rectangles one by one for each cell
...
#+name: TableLayouter-resequenceChildren
#+begin_src dart
 /// Resequence children in order specified by the [TableLayoutDefiner] member [tableLayoutDefiner]
  List<BoxContainer> _resequenceChildren() {
    /// Resequence children in order specified by the [TableLayoutDefiner] member [tableLayoutDefiner]
    List<BoxContainer> resequencedCells = [];
    TableLayoutCellDefiner element = tableLayoutDefiner.firstCellDefinerToLayout;
    TableLayoutCellDefiner? next;
    do {
      resequencedCells.add(element.cellForThisDefiner);
      next = element.nextCellDefinerInLayoutSequence;
    } while (next != null);
    
    assert (resequencedCells.length == _children.length);
    assert (resequencedCells.length == tableLayoutDefiner.numRows * tableLayoutDefiner.numColumns);
    return resequencedCells;
  }
#+end_src

#+name: validateLengthsUsingReduce
#+begin_src dart
  void TableLayoutDefiner-validate {
...
#+name: TableCellDefiner-validate
#+begin_src dart
    ...
    // todo-00-last : incorporate to crossvalidate
    for (int row = 0; row < cellDefinersRows.length; row++){
      ...
      for (int column = 0; column < currentColumn.length; column++){
        ...
        // Validations all rows must have the same length
...
#+name: TableLayoutCellsDefiner-another-validate-and-set
#+begin_src dart
