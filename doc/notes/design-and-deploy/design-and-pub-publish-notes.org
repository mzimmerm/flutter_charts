...
2. [ ] Each BoxContainer must build itself and immediate children in ~buildContainerOrSelf~. Details: do this by calling ~addChildToHierarchyDeprecated~, then, for all children calling ~child.buildContainerOrSelf(this)~
...
6. [ ] *Important: actual children of container : Ensure (manually for now), that each ~BoxContainer~*:
   - EITHER wraps it's actual children in auto-layouter (~RowLayouter or ColumnLayouter~), and uses non-overriden default ~newCoreLayout~. This means that the container has only *ONE IMMEDIATE CHILD* (~RowLayouter or ColumnLayouter~), the IMMEDIATE CHILD ~RowLayouter or ColumnLayouter~ then has *ONE OR MANY* actual children.  
   - OR     adds its' actual children directly, and uses overriden ~newCoreLayout~ which lays out children manually in code, similar to what we do in old ~layout~
7. [ ] Leaf ~BoxContainer~ does not need to implement neither ~buildContainerOrSelf~ (super returns this) nor  ~newCoreLayout~ (super returns when isLeaf). So leaf only needs to implement ~paint~ and ~get layoutSize~
8. [ ] Allow 0, 1 children to be greedy in MainLayoutAxis, otherwise exception
9. [ ] OLD
#+BEGIN_SRC dart
...
Debugging notes and suggestions:
...
- [ ] Debug layouting:
  - [ ] Place some packing or lineup so that legend is off the chart
  - [ ] Debug behavior, make sure it throws exception wher beyound painted rectangle somewhere
- [ ] BoxContainer
  - [ ] Add member key on constructor (nullable) and add to constructors at least those used in Legend
...
- [ ] Add enum ConstraintDivisionToChildren : even, ratios, passFull,
- [ ] Constraint : add method List<Constraints> divideUsingStrategy(ConstraintDivisionToChildren) only implement even If ratios, param must be set, if passFull, return same - plyg that in code first.
- [ ] also expandToConstraintMax=false is NOT compatible with packing = flow
...
- [ ] Add ability to set packing, snap, expandToConstraintMax, constraintDivisionToChildren  on RowLayouter.


- Add 

- [ ] Resolve why RowLayouter Packing.loose behaves same as Packing.snap !! 
- [ ] MILESTONE !!!!!! vvvvvvv 
- [ ] Read https://blog.logrocket.com/flutter-layouts-guide-margins-padding/
- [ ] Create proper RowLayouter, add comments
  - [ ] Basically, need to make sure I can pass constructor arguments to set pack/loose/align - see what they are
- [ ] Make sure default row layouter on container is Row, which aligns left and top (basicall, sets parent offset = 0!!
- [ ] Create proper ColumnLayouter, add comments
- [ ] Create PaddingLayouter and use around legend internals and around legends
