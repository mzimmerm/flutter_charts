...
*** We get rid of the parent sandbox
...
- [ ] all values that must be set by parent, should have
  - [ ] 'apply' methods on BoxLayouter, which act as setters (e.g. applyParentOffset) and pass 2 values:
    - [ ] (Object caller, value)
    - [ ] these will generally check that parent == caller
  - [ ] supporting elements (e.g. _offset) - this SHOULD be final on parentOrderedToSkip, constraints, layoutSize but NOT _offset which needs to keep changing.
  - [ ] getter which returns the private (e.g. _offset)

What are those?
- [ ] offset - does not even need a getter. 
- [ ] parentOrderedToSkip - rename orderedSkip
- [ ] constraints
- [ ] layoutSize - this does NOT have apply - just late final layoutSize
