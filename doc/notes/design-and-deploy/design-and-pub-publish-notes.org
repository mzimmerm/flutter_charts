#+OPTIONS: toc:3
#+TODO: TODO IN-PROGRESS IN-PROGRESS-NOW LATER HOLD DONE-THIS-ITERATION DONE NOTE
...
*** DONE ~USING: Implemented Layouter algorith #2~
...
*** DONE-THIS-ITERATION Core Layouter Rules

1. LAYOUT COMMITS TO A SIZE WITHIN CONSTRAINT; PAINT MUST COMMIT TO THE SIZE FROM LAYOUT.
2. 
3. 
...
** IN-PROGRESS-NOW 4. 2022-10-10 Review
...
IMPORTANT: ~BoxContainer~ extends ~BoxLayouter and BoxContainerHierarchy mixins~, ONLY ADDS METHOD ~paint(Canvas)~ to ~BoxLayouter~
...
*** DONE review of todos

- todo-00-last-last-last : review where we are
- todo-00-last-last : 
- todo-00-last : are there any? Yes, about 10
- todo-00 : There are 2
- *Use todo-done-00 todo-done-01 etc. Here the 00, 01, 02 indicates order, not priority*
- todo-01-document


** TODO 5. Use the new Layouter algorithm + new Box/Column ON SOME Containers (LegendContainer) instead of custom code. REST of Containers (*which*) use original code.

*** NOTE ~New Layout on Containers used in LEGEND = LegendContainer, LegendItemContainer, LabelContainer~
...
- [ ] LabelContainerOriginalKeep (used in both LEGEND and AXIS labels)
- [ ] LegendLabelContainerNewLegendSpecificKeep

- [ ] LegendIndicatorRectContainer - NEW CLASS TO INDICATE LEGEND RECTANGLE.


Containment structure (contained children created in layout()):
...
*** NOTE ~Old Layout on SOME Containers = IN-PROGRESS-NOW~

*** TODO Use RowLayouter in LegendContainer, LegendItemContainer, LabelContainer

** TODO 6. REST Containers : Use the new Layouter algorithm + new Box/Column layouts instead of custom code
