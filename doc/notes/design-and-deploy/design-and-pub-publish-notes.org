...
*** IN-PROGRESS-NOW Make script changes and duplicate 2 test files
...
3. [X] TEST FOR SAME
4. [-] USE 2 TEST APPS : ONE FOR auto_layout, OTHER for coded_layout with the eventual goal of coded_layout gone.
   ...
   3) [ ] Modify run-all-tests.sh and related scripts to accept auto_layout and coded_layout options separately
      1. [ ]  run-all-tests.sh --auto-layout="ex1 ex2 group1 group2" --coded-layout"ex1 ex3 group3"
         + [ ] In processing, of run_all_tests.sh:
           - [ ] no change
         + [ ]  In processing, of run_screenshots_create_then_validate_integration_test.sh
           - [ ] check for --coded-layout and --auto-layout, pick the codedLayoutExampleDescriptors and autoLayoutExampleDescriptors
           - [ ] Run flutter drive 2x, separately for coded_layout and auto_layout, passing the appropriate example descriptors.
         + [ ] 
   4) [ ] TEST FOR SAMENESS AND COMMIT
      + [ ] Delete the tested files first, to make sure all images are generated
      + [ ] run_all_tests.sh --auto-layout="allSupportedNew convertedToNew" --coded-layout="oldFailingInNew"
      + [ ] COMMIT
   5) [ ] NEXT STEP IS INTRODUCE A COPY WITH MINOR CHANGE OF THE TEST FILES FOR CODED_LAYOUT AND AUTO_LAYOUT, SO THAT WE ONLY HAVE TO MAINTAIN ONE VERSION:
      1. [ ] In run_screenshots_create_then_validate_integration_test.sh before calling flutter drive:
         * [ ] copy test_main.dart to coded_layout_test_main.dart with sed editing DOING NOTHING; LATER THIS WILL MAKE SURE CODED_LAYOUT VERSION IS CALLED
         * [ ] Copy screenshot_create_test.dart to coded_layout_screenshot_create_test.dart with sed editing REPLACING
...
*** TODO-NOW Remove SwitchViewModels, switch_view_model.dart, LiveOrTesting
