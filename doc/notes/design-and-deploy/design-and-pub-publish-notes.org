...
- [ ] Add check to newCoreLayout, if RowLayouter OR ColumnLaoter is deeper, and Lineup set to anything but 'start' and Packing 'snap', replace the Row/ColumnLayouter with a new copy with 'start' and 'snap'. Probably at the beginning of newCoreLayout on root?
  - [ ] Test it on some combinations in legend layout
    - [ ] nested Lineup.end on Row
    - [ ] we do not have Column example to test
- [ ] Tests work
...
- [ ] Add container Greedy, equivalent of Expanded with ratio
  - [ ] Change layout algorithm to process non-expanded first, GIVE EXPANDED RATIO OF CONSTRAINT - THIS REQUIRES A NEW STEP
  - [ ] Tests work!!!
  - [ ] CANNOT TEST YET, UNTIL WE CONVERT THE WHOLE CHART TO NEW LAYOUT. 
- [ ] Add Padding BoxContainer that can be used around boxes to pad with space, no painting needed?
- [ ] Add Layout which wraps! (maybe that is called FlowLayout?). This is important for long legends.
