...
*** NOTE YLabels and their layout in flutter_charts

ChartData yLabels and dataRowsColors behave differently.

reason is, yLabels have 2 modes:
- When passed to constructor, then they are set to non-null in argument, then manual layout is used.
- When not passed, they are not used at all! Instead an auto layout of Y is used as follows:
...
*** Make RandomData extend ChartData again.

*** Can we pull util_data methods back on ChartData as members? OR SEE NEXT

*** Try to make members on ChartData private instead of final, and provide getters.
Does that relieve the dataRowsColors issue?
