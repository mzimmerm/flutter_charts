...
**** DONE Use Row in LegendContainer, LegendItemContainer, LabelContainer
...
#+begin_src dart
  BoxContainer buildContainerOrSelfOLD() {
    ...
    // Prepare the children array of actual children which will be passed to the immediately contained Row
    ...
    */
    ...
    // Prepare Row which will wrap the actual children of this [LegendContainer]
    Row row = Row(children: children);

    addChildToHierarchyDeprecated(this, row);

    for (BoxContainer child in row.children) {
    ...
    /* todo-done-00 : Nothing layout specific, we do autolayout.
        Instead, do this:
          ...
          - crate and return new Row with all the children in it
          - return the new Row
...
** DONE 5.2 Layouter improvements DONE : Debugging, Row Packing, Align, settings, overflow rectangle, Greedy, and more
...
** DONE 5.2 Layouter improvements : New Layouters (Greedy, Center, Padding), Row: Packing, Align settings; overflow rectangle, and more
...
#+BEGIN_SRC ditaa :file img-babel-ditaa-1.png :cmdline -E -r
...
** IN-PROGRESS Sequence and structure of calls during old and new layout
...
** IN-PROGRESS 6. REST Containers 1. : ~HUGE: First migrate all existing Containers to use new Layout methods, but still use manual layout as is. Do NOT deal with Presenter, VerticalBarPresenter, LineAndHotspotPresenter~
...
*** NO-NO-NO Options - turn into a singleton, and access it that way everywhere. This probably changes the code a lot, including main.

Options are NOT a singleton. Multiple charts may be used in an application, each with a different ChartOptions

- [X] Run all tests in eshell
- [ ] ChartOptions: How do they get in their lexical scope now? 
  - [ ] ~class structure~ : LineChart( LineChartPainter( LineChartRootContainer ( ChartData ( ChartOptions
  - [ ] *Any other class that needs ChartOptions, gets it either 1) via knowing about LineChartRootContainer, or 2) by being constructed in a place that knows aobut ChartOptions, and passing ChartOptions to constructor*
  - [ ] Good example of 2) is ~LegendItemContainer~, constructed in ~LegendContainer._legendItems~ which passes options from it's LineChartRootContainer to  ~LegendItemContainer~ constructor.
  - [ ] Most classes get options by holding on ChartRootContainer and accessing Options through ~ChartRootContainer -> ChartData data -> ChartOptions chartOptions~. Some classes 
  - [ ] Classes that hold on ChartRootContainer: look for ~required ChartRootContainer chartRootContainer~  *this is also a good list of all important classes for refactoring*
    - [ ] ChartRootContainer.createDataContainer()
    - [ ] YContainer                                         constr
    - [ ] XContainer                                         constr
    - [ ] AdjustableLabelsChartAreaContainer                 constr
    - [ ] DataContainer                                      constr
    - [ ] VerticalBarChartDataContainer                      constr
    - [ ] LineChartDataContainer                             constr
    - [ ] LegendContainer                                    constr
    - [ ] Presenter                                          constr
    - [ ] PresentersColumns                                  constr
    - [ ] VerticalBarPresenter                               constr
    - [ ] LineAndHotspotPresenter                            constr
    - [ ] PresentersColumn                                   constr
    - [ ] PresentersColumn._createPresentersInColumn
    - [ ] PresenterCreator.createPointPresenter
    - [ ] VerticalBarChartRootContainer.createDataContainer 
    - [ ] VerticalBarLeafCreator.createPointPresenter
    - [ ] LineChartRootContainer.createDataContainer
    - [ ] LineAndHotspotLeafCreator.createPointPresenter
    - [ ] 
    - [ ] 
  - [ ] Classes that hold on ChartOptions      : look for ~required ChartOptions options~
    - [ ] LegendItemContainer                                constr
    - [ ] LegendIndicatorRectContainer                       constr           
    - [ ] DefaultIterativeLabelLayoutStrategy                constr
    - [ ] LabelContainer                                     constr
    - [ ] AxisLabelContainer                                 constr
    - [ ] RandomChartData                                    constr
    - [ ] 
- [ ] Study singleton in dart and select one version to use
- [ ] In ChartOptions and extensions :
  - [ ] Add singleton-defining methods
  - [ ] Can ChartOptions it stay const?
- [ ] All BoxContainer extensions which pass options, comment out
- [ ] In main, inside the big switch, add an expression which creates instance of the appropriate OptionsSingleton.
- [ ] In code, whereever there is compile error on now missing options, set it from the singleton
- [ ]
- [ ] 
- [ ] 
- [ ] 
...
*** IN-PROGRESS-NOW Go over existing BoxContainers, and make sure children are created before parent
*** IN-PROGRESS-NOW Make sure each BoxContainer has a key, unique key amonng siblings

- [ ] Key -> SiblingsKey (like LocalKey) -> SiblingsValueKey (ValueKey)
- [-] On BoxContainer -  Guarantee that each child has a unique key amonng siblings
  - [ ] 
  - [X] Create immutable class Key, member value
  - [-] Key
    - [X] Add member late final Key key to BoxLayouter. Value 
    - [X] method ensureUniqueWith((List<Key> keys) - throw exception if not
    - [X] method Key uniqueNextWith(List<Key> keys) 
      - [X] generate name as childN N = keys.size + 1, ID as N
    - [ ] When adding children to BoxContainer:
      - [ ] if child key is not null, call ensureUniqueWith((List<Key> keys)  if it's unique among children before it. If null, set to uniqueNextWith(List<Key> previousChilren.keys)  
...
- [ ] BoxContainer and all other constructors that have children: param, remove setting children, and set _namedChildren instead, generatinve the name = child1, child2 etc.
...
- [ ] commit and push
*** IN-PROGRESS-NOW Move container.dart and related container classes to a new directory container_layouter

*** IN-PROGRESS-NOW Study use of container-hard-to-convert classes:  ~Presenter - VerticalBarPresenter, LineAndHotspotPresenter~

What would it take to convert them to BoxContainer extensions?

Capture the process here:
...
- [ ] List of all classes: Mark as candidate (C) or later (L) never (N) to convert to BoxContainer
...
- [ ] containers or container-like to leave for later.
    ...
    - [ ] LegendContainer
    - [ ] abstract AdjustableLabelsChartAreaContainer
    - [ ] Presenter
    - [ ] PresentersColumns
