...
*** IN-PROGRESS ~Use the new Layout on Containers used in LEGEND = LegendContainer, LegendItemContainer, LabelContainer~

**** TODO Use RowLayouter in LegendContainer, LegendItemContainer, LabelContainer

**** IN-PROGRESS-NOW Continue new Layout on LEGEND

- [ ] *in build, try to nest like in Flutter*
  - [ ] *FOR THAT, WE NEED TO*
    - [ ] ENFORCE ALL BoxContainer CONSTRUCTORS TO ACCEPT REQUIRED 'CHILDREN'*
    - [ ] Give BoxContainer a factory constructor, and move code from buildContainerOrSelf to the factory constructor
      - [ ] To do this:
        - [ ] remove argument parentBoxContainer from ~buildContainerOrSelf(BoxContainer parentBoxContainer)~. It can be accessed via parent anyway!
        - [ ] Rename  ~buildContainerOrSelf()~ to factory ~buildContainerOrSelf()~
- [ ] 
- [ ]
