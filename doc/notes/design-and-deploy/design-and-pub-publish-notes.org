...
** DONE 11.1. Rename to outer

Wherever there is a natural hierarchy of inner classes, where outer creates inner in an abstract way, use this term for the method that creates there inner instance:

- [X] Make all fields named 'owner' to be final; also rename them to 'outer'
  - [X] ownerDataColumnPointsBar => outerDataColumnPointsBar : also check if it is set as member and used in some way!!!
  - [X] ownerBarsContainer => outerBarsContainer : also check if it is set as member and used in some way!!!
  - [X] ownerDataContainer => outerDataContainer
- [X] Look for OWNER, consider if it needs rename to outer. Usually, this should be accompanied with a 'make' method, which should be renamed to 'makeInner'. FINISH THIS. LOOK FOR OWNER, BUT NOT IN unused.dart

** IN-PROGRESS-NOW 11.2. ~CURRENT WORK: ADD LINE CHART~

Bar Chart:

- [X] Rename LegendColumn to ByRowLegends
  - [X] Rename LegendColumn to ByRowLegends
- [X] Refactor DataContainer in BarChart:
  - [X] Bar chart: DataContainer : create extension BarChartDataContainer in container/bar/data_container.dart:
    - [X] Use the existing mechanism, override the 3 methods that throw UnimplementedError, this does not need to override all data_container classes, and keep the core layout intact
      - [X] makeDataColumnPointsBar
      - [X] makePointContainer
      - [X] makePointContainerWithZeroValue
    - [X] Get Bar Chart working again in the new structure


Line chart:

- [-] Line Chart
  - [X] Simplify code, remove extra comments
  - [ ] Figure out to paint circle in the middle of the bar!!
  - [ ] Abstract out the affmapToPixelsMaybeTransposeInContextOf to interface, which is both on PointModel and PointOffset, returning PointOffset.
    - [ ] In PointModel, it first creates PointOffset WITH INDEPENDENT (INPUT) VALUE IN THE MIDDLE OF CONSTRAINT, then calls the PointOffset version
    - [ ] In PointOffset, as is.
  - [ ] Get orientation=row working
  - [ ] Add lines connecting points to both column and row
- [ ] Go over todo-010 on Bar Chart first and implement generalizations:
  - [ ] The zero points - simplify for both Bar and Line
  - [ ] Figure out the generalization split into a new Inner maker
- [ ] Can we get stacked Line chart working? What would it take?
  


Two more things:

- [ ] problem: The affmapToPixels is only called in CL code. Need to fix this first!!! - maybe use the new method affmapValuesToPixels instead??
- [ ] The above vs affmapValuesToPixels!!! compare them



** LATER 12. ~CURRENT WORK: BACKUP AND LONG TERM CAPTURED TODOS IN LAYOUT~
...
** TODO Remove all usages of Model classes in data_container.dart files. Only talk to ChartViewModel.

Not sure if to do this ... Need to think through the separation of View and ViewModel.

- [ ] In common view_model.dart add classes that mirror ChartModel:
  - [ ] ChartViewModel for ChartModel (already done)
    - [ ] Constructed from ChartModel
    - [ ] It's api is same as ChartModel - todo - check what this means
  - [ ] LegendItem with ChartLegendItem
  - [ ] DataColumnModel with DataColumnViewModel
  - [ ] PointModel with PointViewModel
- [ ] Search all  *data_container*dart for import of data_model.dart
  - [ ] replace all imports, and usages of classes from  data_model.dart with corresponding classes from view_model.dart

** TODO FUTURE WORK

- [ ] Create a test app that can:
  1. [ ] delay
  2. [ ] take screenshot from the app,
  3. [ ] delay
  4. [ ] run another chart
  5. [ ] back to 1.
- [ ] Add spacing
