...
** IN-PROGRESS-NOW ~13. Create a test app for super fast testing~ 2 days, but be willing to give up. Should start May 23, started May 24. Should go: May 23, May 24.

- [ ] Create a test app that can:
  1. [ ] run first chart in list
  2. [ ] delay 2s
  3. [ ] take screenshot from the app,
  4. [ ] delay 2s
  5. [ ] run another chart
  6. [ ] back to 1.
- [ ] Add spacing
...
- [ ] Modify/copy the existing main.dart, to ~main_new.dart~, in main_loop_charts.dart add a button 'next' instead of + button
  - [ ] Clicking 'next' will run another test, from a list of tests (ex10-ex900, different combinations of chart, orientation, stacking
    - [ ] start with 2 tests in the list
  - [ ] Make 'next' disabled after last test is run
- [ ] Modify/copy the existing driver test ~screenshot_create_test_new.dart~
  - [ ] Run as
  - [ ] Add a loop around the 'pump' until after screenshot is taken
  - [ ] After screenshot is taken, add code mocking clicking the 'next', after that, go to top of loop, before 'pump and settle'
  - [ ] Loop end condition is that 'next' is disabled


Later:

- [X] Rename screenshot_check_test and similar to validate_screenshot_test or maybe test_validate_screenshot??
-
...
** LATER ~add WrappingLayouter.Column/Row, similar to but simpler than Column or Row.~ 3 days. Should start May25, end May27

Column: lays out left to right, when reaches constraints width,  wraps to next line, starting to the left of 'max height'
Row: lays out top to bottom, when it reaches constraints height, wraps to top,       starting to the righ of 'max width'

Use it in one example. 


** LATER ~add dotted grid lines in it's own StackLayouter, then place axis lines into it's own container~ 4 days. Should start May28, end June 1.


** LATER ~figure out how to create and keep some basic layout templates - e.g. to pre-layout Y axis~ 4 days. Should start June 2, end June 5.
