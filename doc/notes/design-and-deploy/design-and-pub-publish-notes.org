...
** IN-PROGRESS Replace ChartData and add DataPoint, more representing what people need, and adding logarithic scale

*** NOTE YLabels, YScaler, and their layout in flutter_charts
...
**** IN-PROGRESS-NOW Add unit tests for the following unclean classes and methods, before refactoring them
- util_dart.scaleValue  
- Range._distributeLablesIn(Interval) # Called in makeYScalerWithLabelInfosFromDataYsOnScale. So if this fails, the latter should fail as well
- Range.makeYScalerWithLabelInfosFromDataYsOnScale
- YScaler.scaleLabelInfos
- YScaler.makeLabelsPresentable


Some results to add to tests:

I/flutter ( 4897): for_scaleValue_test: [0.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
286.7321428571429]
I/flutter ( 4897): for_scaleValue_test: [2000.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
33.33928571428572]
I/flutter ( 4897): for_scaleValue_test: [-600.0, -1000.0, 2200.0, 413.42857142857144, 8.0, 362.75]
I/flutter ( 4897): for_scaleValue_test: [0.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
286.7321428571429]
I/flutter ( 4897): for_scaleValue_test: [-600.0, -1000.0, 2200.0, 413.42857142857144, 8.0, 362.75]
I/flutter ( 4897): for_scaleValue_test: [600.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
210.71428571428572]
I/flutter ( 4897): for_scaleValue_test: [0.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
286.7321428571429]
I/flutter ( 4897): for_scaleValue_test: [600.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
210.71428571428572]
I/flutter ( 4897): for_scaleValue_test: [1400.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
109.35714285714289]
I/flutter ( 4897): for_scaleValue_test: [600.0, -1000.0, 2200.0, 413.42857142857144, 8.0,
210.71428571428572]


Also there is this error running all tests:

+ echo Running flutter test for EXAMPLE_TO_RUN=ex32AllPositiveYsYAxisStartsAbove0, CHART_TYPE_TO_SHOW=lineChart.
Running flutter test for EXAMPLE_TO_RUN=ex32AllPositiveYsYAxisStartsAbove0, CHART_TYPE_TO_SHOW=lineChart.
+ flutter test --dart-define=EXAMPLE_TO_RUN=ex32AllPositiveYsYAxisStartsAbove0 --dart-define=CHART_TYPE_TO_SHOW=lineChart test/screenshot_check_test.dart
00:06 +0 -1: after screenshot integration, test for sameness [E]                                      
  FileSystemException: Cannot open file, path = 'integration_test/screenshots_expected/ex32AllPositiveYsYAxisStartsAbove0_lineChart.png' (OS Error: No such file or directory, errno = 2)
  dart:io                                _File.readAsBytesSync
  test/screenshot_check_test.dart 35:22  main.<fn>
  
00:06 +0 -1: Some tests failed.                                                                       

*I think I renamed an enum, rename the files*

**** IN-PROGRESS-NOW *DESIGN OF LOGARITHMIC SCALING WITH EXISTING LABEL FORMATTERS AND SCALERS*

***** Logarithms of base 10 and e
...
~log10(x) = log(x) / ln10~ where log(x) is loge(x)

***** Steps to introduce logarithmic scale
- [X] Add ChartOptions.DataContainer.yTransform Function double -> double for now (yTransform applies to both DataContainer and YContainer) 
- [ ] *everything in LabelInfo is untransfered*
- [ ] *CREATE TERMINOLOGY FOR LABELS: LABEL NOT TRANSFORMED Y VALUE, LABEL TRANSFORMED Y VALUE, LABEL Y AXIS POSITION*

- [ ] *problem* notTransformedDataY is WRONG, it is 10.0 where it should be 1000000 or so. Main reason: labelYsInDataYsEnvelope (this is 0 .. 12) is not where LabbelInfos should be constructed from

- [ ] *problem* : because we start scale always at 0.0, when notTransformedDataY=0.0, then
  - transformedDataY = yTransform(notTransformedDataY); is INFINITY

- [ ] *note* : ~labelInfo.formattedYLabel is printed~

  
- [ ] Rename:
  
  - [ ] GET dataYLabelValues  according to above naming.
  - [ ] notTransformedScaledDataY according to above - this is value on Y axis
  - [ ]labelValuesInDataYsEnvelop to notTransformedLabelValuesInTransformedDataYsEnvelop
  - [ ] dataYsLabelValues to notTransformedDataYsLabelValues
  - [ ] labelInfos to notTransformedLabelInfos
- [ ] *PROBLEM: IN STACKED CHART, WE FIRST PASS LOG(Y), THEN ADD LOG(YS) DURING STACKING. THAT IS WRONG. THE STACKABLE COLUMNS MUST FIRS ACCEPT Y, THEN ADD DURING STACKING, THEN DO LOG(ADDED).*
- [ ] only applies if yValues are numeric and >= 0, otherwise exception, method: checkDomainAndRangeValidFor(yTransform)
- [ ] only applies on line chart? = both for now
- [ ] but when creating labels, values passed to the formatter and scaler is yValue! so it sort of works like manual yLabels
- [ ] Later
  - [ ] implemented by ovewriting DataSetsMixin.yTransform (move this to later part)
  - [ ] document dataY is always trasfered, maybe name that way  
- [ ] *maybe i implement this first in existing system using option dataContainerOption.logarithmicYValues to learn how to do this*
...
*** TODO ChartData further refactoring
