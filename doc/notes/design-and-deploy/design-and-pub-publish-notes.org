...
12. [ ] row orientation: Check it out - ONLY IN NEW AUTOLAYOUT. How does it look??
    - [ ] Probably some issues with transposed X, Y axis? Analyse and debug.
      - [ ] The layouter is outside all bounds. No traces of X or Y labels
      - [ ] 
...
16. [ ] Work on tests : Change the testing framework to only BUILD ONCE then run multiple tests.
