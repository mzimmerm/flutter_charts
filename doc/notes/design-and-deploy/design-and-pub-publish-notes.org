...
*** IN-PROGRESS-NOW Add note that Envelop always means Envelope of List<double>. It is always an Interval, either a closure of the List<double>, or it's extension to start or end at 0.
*** IN-PROGRESS-NOW Rename any _container, container members to _topContainer if it is actually ChartableTopContainer
*** IN-PROGRESS-NOW LabelInfo: Make some parts a getter rather than member

- [ ] Convert LabelInfo.rawDataValue to getter
- [ ] Convert LabelInfo.axisValue to getter

*** IN-PROGRESS-NOW Rename DataRows to RawDataRows, DataRow to RawDataRow.
...
*** TODO *Allow to reach to the top level chart by passing a baseclass Chart to top container. From there, both chart data and chart options can be reached.*
