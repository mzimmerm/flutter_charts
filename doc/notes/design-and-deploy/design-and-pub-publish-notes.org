...
- [!] also expandToConstraintMax=false is NOT compatible with packing = flow - add that note somewhere
- [ ] Layouter general : Add to OneDimLayoutProperties expandToConstraintMax: true and false, default true. Tests should work
  - [ ] If true, use available constraint (totalSize). BUT always try false in code.
- [ ] Layouter general : Add to OneDimLayoutProperties constraintDivisionToChildren: default 
- [X] Add ability to set packing, snap, expandToConstraintMax, constraintDivisionToChildren  on RowLayouter and ColumnLayouter.
- [ ] BoxContainer debugability
...
- [X] Add check to newCoreLayout, if RowLayouter OR ColumnLaoter is deeper, and Lineup set to anything but 'start' and Packing 'snap', replace the Row/ColumnLayouter with a new copy with 'start' and 'snap'. Probably at the beginning of newCoreLayout on root?
  - [X] Test it on some combinations in legend layout
    - [X] nested Lineup.end on Row
    - [X] we also have Column example to test
- [-] One remaining todo-00-last-last
  - [X] Find problem or comment
  - [ ] Make sure Tests work
  - [ ] Commit and push
- [ ] Add ability in Main and in LegendContainer to create example which overrides creation of ~LegendContainer~
  - [ ] In this code, using the previously switch-set chartData and xContainerLabelLayoutStrategy we create the Widget for the chart enum
    #+begin_src dart main-createRequestedChart-end
    Widget chartToRun;
    ...
    return chartToRun;
    #+end_src
  - [ ] We need the ability to define different LegendContainer and LegendItemContainer and a way to push from this code which one we want.
    - [ ] Pushing: via ChartOptions.legendOptions
      - [ ] New enum LegendAndItemLayoutEnum
        - [ ] legendIsRowStartSnapItemIsRowStartSnap
        - [ ] legendIsRowStartSnapItemIsRowEndLoose
        - [ ] etc
      - [ ] ChartOptions.LegendOptions
        - [ ] add member LegendAndItemLayoutEnum legendAndItemLayoutEnum
        - [ ] set some default
      - [ ] In main enum switch
        - [ ] Add 4 examples enum
        - [ ] set the new enum LegendAndItemLayoutEnum on chartOptions in main around
          #+begin_src dart
          ...
          break;
          #+end_src
    - [ ] Implementing: in LegendContainer and LegendItemContainer 
      - [ ] LegendContainer.buildContainerOrSelf, change to
        #+begin_src dart
          // Pulled from inline to allow different Wrapper
          BoxContainer _legendItems(some params needed) {
            return  [
          ...
          BoxContainer buildContainerOrSelf() {
            ...
            switch(options.legendOptions.legendAndItemLayoutEnum.legendAndItemLayoutEnum) {
              case LegendAndItemLayoutEnum.legendIsRowStartSnapItemIsEndLoose:
                return
                  RowLayouter(
                    mainAxisLineup: Lineup.start, // Lineup.start (for tests)
                    mainAxisPacking: Packing.snap, // Packing.snap (for tests)
                    children: _legendItems(some params here),
                  );
                etc case
           }
          }
        #+end_src 
      - [ ] LegendItemContainer.buildContainerOrSelf - similar changes
  - [ ] Make sure existing tests work !!! - using the default legend enum legendIsRowStartSnapItemIsRowStartSnap
- [ ] Add 3 examples (only use LineChart) which overrides creation of ~LegendContainer~
  - [ ] LegendContainer starts with RowLayouter    'start',  'snap',  LegendItemContainer is Row 'start', 'snap' (default desired and tested)
  - [ ] LegendContainer starts with RowLayouter    'center', 'loose', LegendItemContainer is Row 'end', 'loose' (overriden to 'start', 'snap' , so noop)
  - [ ] LegendContainer starts with ColumnLayouter 'start',  'snap',  LegendItemContainer is Row 'start', 'snap' (is top, so is NOT overriden, so must be as intended!)
  - [ ] LegendContainer starts with ColumnLayouter 'start',  'loose', LegendItemContainer is Row 'start', 'snap' (as above)
  - [ ] Create new test images, check them
  - [ ] Add the above to representative tests in ~ExamplesDescriptor~ (line charts only)
  - [ ] Run all tests
  - [ ] commit and push
- [ ] *Add proper layouter and container hierarchy for offsetting and non-offsetting layouters. In the end, there should be nothing related to ~main/crossAxisLayoutProperties~ in BoxLaouter or BoxContainer*
  - [ ] ~BoxContainer~ will serve as a default ~NonOffsettingBoxLayouter~
    - [ ] Find the method ~YYY~ which does layout of children and performs their offset - must be using ~main/crossAxisLayoutProperties~
    - [ ] override the method ~YYY~ from BoxLayouter in ~BoxContainer~
      - [ ] It must do perhaps nothing? or just applyParentOffset to 0.0? or just call super.applyParentOffset?
  - [ ] Add abstract ~OffsettingBoxLayouter~ extends ~BoxContainer~ - empty impl
  - [ ] Add abstract ~RollingOffsetBoxLayouter~ (this is like Flex) extends ~OffsettingBoxLayouter~
  - [ ] Make ~Row and ColumnLayouter~ extend ~RollingOffsettingBoxLayouter~
    - [ ] Move ~YYY~ from BoxLayouter to ~RollingOffsetBoxLayouter~
    - [ ] Move ~mainAxisLayoutProperties and many friends (which? list here first)~ from BoxLayouter to ~RollingOffsetBoxLayouter~
      - [ ] friend 1
      - [ ] friend 2
  - [ ] *In the end, there should be nothing related to ~main/crossAxisLayoutProperties~ in BoxLaouter or BoxContainer*
  - [ ] Make sure Tests work
  - [ ] commit and push
