...
**** IN-PROGRESS-NOW Move ~buildContainerOrSelf~, move contents to BoxContainer constructor as children. Remove this.children setter
...
3. [ ] remove ~buildContainerOrSelf~ from comments
4. [ ] Remove ~this.children~ from code and everywhere. Any consequences???? Look at ChartRootContainer returning [legendContainer, yContainer, etc]
5. [ ] run minitest 
...
**** Can we make ~ensureKeyedMembersHaveUniqueKeys~ private?
