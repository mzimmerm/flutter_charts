...
** DONE 5. STUB - LEGEND LAYOUT: Use new Layouter algorithm + new Box/Column wrap ON LEGEND Containers (LegendContainer) instead of custom code. REST of Containers (ChartRootContainer, X/YContainer, DataContainer) use original code.

** DONE 5.1 STUB - Members which should always be set by parent, use name 'applyParent', and add explicit caller test

** DONE 5.2 Layouter improvements (long but keep)
...
*** IN-PROGRESS-NOW KEEP : Ensure code sets up the BoxContainer parent/child hierarchy correctly in existing containers ~EXCEPT Presenter - VerticalBarPresenter, LineAndHotspotPresenter~

Setting up container hierarchy is crucial, because the next step, 
...
- [X] Fully converted to new layout - no manual layout used:
  - [X] LegendContainer
...
- [X] Add ~BoxContainerUsingManualLayout~  which throws exception from ~post_NotLeaf_PositionChildren~. Extensions must override this method by moving position-calculations from ~layoutOLD~ to this method.
- [ ] Add  class ~ChartAreaContainerUsingManualLayout extends BoxContainerUsingManualLayout~
- [X] Make these extend ~BoxContainerUsingManualLayout~ instead of ~BoxContainer~ This forwards layout to manual layout
  - [X] ChartRootContainer
  - [X] ChartAreaContainer
  - [X] YContainer                            (via extends ChartAreaContainer)
  - [X] XContainer      (via extends AdjustableLabelsChartAreaContainer extends ChartAreaContainer)
  - [X] DataContainer   (via extends ChartAreaContainer)
  - [X] AdjustableLabelsChartAreaContainer    (via extends ChartAreaContainer)
...
- [ ] Make these ~layout~ code, after creating a child, keep in children, and at the end, add children to self
  - [ ] ChartRootContainer
  - [ ] ChartAreaContainer
  - [ ] YContainer                            (via extends ChartAreaContainer)
  - [ ] XContainer      (via extends AdjustableLabelsChartAreaContainer extends ChartAreaContainer)
  - [ ] DataContainer   (via extends ChartAreaContainer)
  - [ ] AdjustableLabelsChartAreaContainer    (via extends ChartAreaContainer)
  - [ ] VerticalBarChartRootContainer         (via extends ChartRootContainer)
  - [ ] VerticalBarChartDataContainer         (via extends DataContainer)
  - [ ] LineChartRootContainer                (via extends ChartRootContainer)
  - [ ] LineChartDataContainer                (via extends DataContainer)



*** IN-PROGRESS-NOW KEEP : Old layout and code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~post_NotLeaf_PositionChildren~,  but positioning code is still old, manual, copied from old code!!!

**** Migate these

- [ ] These extend ~BoxContainerUsingManualLayout~ instead of ~BoxContainer~ This forwards layout to manual layout
  - [ ] ChartRootContainer
  - [ ] ChartAreaContainer
  - [ ] YContainer                            (via extends ChartAreaContainer)
  - [ ] XContainer      (via extends AdjustableLabelsChartAreaContainer extends ChartAreaContainer)
...
The above must override ~BoxContainerUsingManualLayout.post_NotLeaf_PositionChildren~.

Extensions must override this method by moving position-calculations from ~layoutOLD~ to this method.

**** Migration process

However, ~newCoreLayout~ can only function if recursive calls to it is applied. So the migrated code must have children setup.
...
*** LATER Study use of container-hard-to-convert classes:  ~Presenter - VerticalBarPresenter, LineAndHotspotPresenter~
...
- [ ] commit and push
*** LATER Go over existing BoxContainers, and make sure children are created before parent
...
** LATER 7. REST Containers 2. : ~Presenter - VerticalBarPresenter, LineAndHotspotPresenter - convert to BoxContainers~
...
** TODO-NEXT 9. EVEN-LATER. : BoxContainer debuggability, introduce Singletons on places
