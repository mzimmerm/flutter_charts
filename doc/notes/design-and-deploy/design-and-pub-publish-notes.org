...
*** TODO-NOW todo-00 Go over coded_layout classes and ensure they are named CL and NOT used outside coded_layout
...
8. [ ] ~LegendContainer~: In ~lib/test/src/~ create directory ~chart~ and a structure that allows to add test-specific ~LegendContainer~ and perhaps other containters. Refactor/copy file with  ~LegendContainer~ to that directory. Now make following changes:
...
9. [ ] Equivalent of the above for ~LegendItemContainer~
