...
*** IN-PROGRESS-NOW KEEP : BoxContainer extensions : Setup parent/child hierarchy in existing containers ~EXCEPT Presenter - VerticalBarPresenter, LineAndHotspotPresenter~
...
The first is used for direct addressing in existing code, the second for iteration and recursion when called from ~newCoreLayout~
...
- [ ] ~layout~ code, do the following changes
...
*** IN-PROGRESS KEEP : ~layoutOLD()~ code and code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~post_NotLeaf_PositionChildren~,  but positioning code is still old, manual, copied from old code!!!
