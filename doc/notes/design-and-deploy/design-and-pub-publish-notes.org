...
- [ ] After tons of layout changes, the grid disappeared from the line chart only.
...
*** 
*** IN-PROGRESS ~HUGE Refactor LayoutExpansion to SizeConstraints~ !!!
...
- [ ] LayoutExpansion is now passed to layout as it functions basically as SizeConstraints!!
-



**** TODO LayoutExpansion : remove all _layoutSize from extensions, and let base Container to have it as member, replacing getter layoutSize in preparation of moving all these basic elements related to layout to Container.

- This should be possible in principle relatively simply.
...
**** TODO ~LayoutExpansion : solve core problem - Currently, we use LayoutExpansion both as Constraints (by passing to layout and holding on to it) as well as layoutSize (by doint things such as get layoutSize { return _layoutExpansion }~
...
**** TODO Rename LayoutExpansion to SizeConstraints (to distinguish from BoxConstraints and SliverConstraints)

**** TODO Rename offset to layoutOffset

***** rename applyParentOffset to applyParentLayoutOffset
...
**** TODO ~Add SizeConstraints~

                                     
- [ ] Add size_constraint.dart -
  - SizeConstraints{ _maxSize = Size.infinity; SizeConstraints({maxSize:}); SizeConstraints.infinity: redirect to SizeConstraints(maxSize: Size.infinity};},
- [ ] In existing code, Pass SizeConstraints to each Container.layout({required SizeConstraints sizeConstraints})
- [ ] Add SizeConstraints field to Container, from constructor.
...
- [ ] add constructor named root
- [ ] consider constructors from children:
- [ ] add members
  - [ ] parent on top, should be root (in ChartContainer)
  - [ ] children
- [ ] add method traverse 
- [ ] add method flatten 
- [ ] add member name not null, required, set in constructor, functions as unique key. Ensure name unique in hierarchy. 
- [ ] add method isUniqueName
- [ ] children should be just a List<Containers>
- [ ] add member root. On top, should be null. 
- [ ] add method addChild(Container)
- [ ] add member sizeConstraints - NOT NOW, LATER - ONLY PASSED TO LAYOUT, NO NEED TO HOLD ON TO - But flutter holds on to constraints in RenderObject 
- [ ] add member layoutSize
- [ ] keep member offset
- [ ] implement default method layout(SizeConstraints) - see [[Layouter general function]]
...
- [ ] applyParentOffset removed (BUT KEEP AT FIRST) - replaced by calculateAndSetActualChildrenLayoutSizesAndOffsetsInSelf
  ...
  - calls applyParentOffset on all children 
...
- Generally, Containers lifecycle should be as follows.
  ...
  - Layout
    - called by root.layout(SizeConstraints)
    - side effects
...
1. *SizeConstraints is passed down* to layout call.                                               parent context: ~Size childPreferredSize = child.layout(sizeConstraintsDefinedByParent)~  <= side effect: child.size is set (=childPreferredSize) 
...
#+BEGIN_SRC python
  def layout(Size sizeConstraintsFromParent):     # 1. sizeConstraintss go down (as this is called in parent context)
    if this is leaf:
      this.layoutSize = calculate own layoutSize (may be e.g. size of rectangle if this is a rectangle) if not withit SizeConstraints, exception???
