...
- ~Volume~ is the shape of Container, after layout
  - ~Object get surfaceBound~
- ~BoxVolume~
  - ~Size get surfaceBound~

- ~Layout~ is base class
  - does NOT store size or shape, only returns Volume from layout 
  - abstract method ~Volume layout(covariant Constraints)~ specializations call this and implement this, probably calling super. This maybe eventually configures some constraints caching and debugging. 

- ~Painter is mixin~ 
  - ~void paint(Canvas)~

- ~Container extends Layout with Painter~
  - slot ~parent~
  - slot ~children~

- ~BoxContainer extends Container~
  - method ~BoxVolume layout(covariant BoxConstraints)~
    - implemented by calling children.layout
