...
* IN-PROGRESS Dart questions running
...
*** TODO Study function of LayoutExpansion and remove the GrowDoNotFill, TryFill, definition, parameters, arguments, and passing down entirely.
...
#+BEGIN_SRC python
  def layout(Size min, Size max):     # 1. constraints go down (as this is called in parent context)
    if this is leaf:
      ...
      for child in children:
        Size size = child.layout(constraintDefinedByParent) # ignore return, we have it in side-effect
