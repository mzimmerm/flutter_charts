...
** IN-PROGRESS-NOW 5.2 Layouter improvements : Debugging, RowLayouter Packing, Align, and other settings
...
- [X] Add ability to set packing, tight, expandToConstraintMax, constraintDivisionToChildren  on RowLayouter and ColumnLayouter.
...
- [X] Add check to newCoreLayout, root section, if RowLayouter OR ColumnLaoter is deeper, and Align set to anything but 'start' and Packing 'tight', replace the Row/ColumnLayouter with a new copy with 'start' and 'tight'.
...
- [X] Add ability in LegendContainer to create example with several different layouts of ~LegendContainer~
  ...
  - [X] Add 3 examples (only use LineChart) which overrides creation of ~LegendContainer~
    - [X] LegendContainer starts with RowLayouter    'start',  'tight',  LegendItemContainer is Row 'start', 'tight' (default desired and tested)
    - [X] LegendContainer starts with RowLayouter    'center', 'loose', LegendItemContainer is Row 'end', 'loose' (overriden to 'start', 'tight' , so noop)
    - [X] LegendContainer starts with ColumnLayouter 'start',  'tight',  LegendItemContainer is Row 'start', 'tight' (is top, so is NOT overriden, so must be as intended!)
    - [X] LegendContainer starts with ColumnLayouter 'start',  'loose', LegendItemContainer is Row 'start', 'tight' (as above)
    - [X] Create new test images, check them
    - [X] Add the above to representative tests in ~ExamplesDescriptor~ (line charts only)
    - [X] Run all tests
    - [X] commit and push
- [X] *Add proper layouter and container hierarchy for offsetting and non-offsetting layouters. In the end, there should be nothing related to ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~ in BoxLayouter or BoxContainer*
  - [X] ~BoxContainer~ will serve as a default ~NonOffsettingBoxLayouter~
    - [X] Find the method ~YYY~ which does layout of children and performs their offset - must be using ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~
      - [X] There are *multiple methods* that address ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~:
  ...
  - [X] Make ~Row and ColumnLayouter~ extend ~RollingOffsettingBoxLayouter~
  ...
  - [X] Make sure Tests work
  - [X] commit and push
- [X] Rename Lineup to Align
- [X] Rename Packing.snap to Packing.tight
- [X] Make sure Tests work
- [ ] Containers should have either single child or multiple children
  - [ ] Add some way to throw exception if Container does not have intended amount
  - [ ] Maybe add a overridable method maxChildren, values either 1 or infinity
  - [ ] Add maxChildren overrides ad appropriate to each container
- [ ] Make sure Tests work
- [ ] from newCoreLayout, pull the whole root processing into a method, so it can be used in overrides. The root processing should generally be same for all containers
- [ ] test representative
- [ ] test all
- [ ] Add container Greedy, equivalent of Expanded with ratio
  - [ ] Greedy:
    - [ ] is offsetting
    - [ ] has a single child
    - [ ] parent must be Row or Column
    - [ ] does expand child in the direction of it's parent: if Row, horizontally, if Column, vertically 
    - [ ] has member int greed - direction is implied by parent : if parent is Row, the Greedy child greed direction is horizontal, for Column it is vertical
    - [ ] its parent (which should be Row or Column) should do in override of ~_post_IfNotLeaf_Offsetting_LayoutMyChildren_Then_OffsetChildrenInMe_AccordingToLayouter~???: NO - WE NEED TO OVERRIDE IN ROW AND COLUMN, THE WHOLE newCoreLayout
      - [ ] Row or Column need to override newCoreLayout completely ???? Their normal loop
        #+begin_src dart
          ...
          _postDescend_IfLeaf_SetMySize_NotLeaf_OffsetImmediateChildrenInMe_ThenSetMySize_Finally_CheckIfMySizeWithinConstraints();
        #+end_src
      - [ ] Should separately process non-greedy first, greedy next
        #+begin_src dart
          _preDescend_DistributeMyConstraintToImmediateChildren_AccordingToLayouter();
          // B. node-descend
          for (var child in nonGreedyChildren) {
            // 1. child-pre-descend (empty)
            // 2. child-descend
            child.newCoreLayout();
            // 3. child-post-descend (empty)
          ...
          if (hasNonGreedy) _postDescend_NonGreedy_IfLeaf_Exception_NotLeaf_DoStuffIn_C(); // same as current on Row and Column

          // D. node-descend
          for (var child in greedyChildren) {
            // 1. child-pre-descend (empty)
            // 2. child-descend
            child.newCoreLayout();
            // 3. child-post-descend (empty)
          ...
          if (hasGreedy) _postDescend_Greedy_IfLeaf_Exception_NotLeaf_OffsetImmediateChildrenInMe_ThenSetMySize_Finally_CheckIfMySizeWithinConstraints();
        ...
        #+end_src
      - [ ] children already have layoutSize, including Greedy child ?????
      - [ ] C: ask NonGreedy children layoutSizes first
      - [ ] C: get the NonGreedy ~layoutSize~(s), and create a ~nonGreedyChildrenSize~ that envelopes the NonGreedy children, as if they were tightly layed out in the appropriate Column/Row direction
      - [ ] C: creates new constraints ~greedyChildrenRemainingConstraint~ which is a difference between self original constraint, and  nonGreedyChildrenSize
      - [ ] C: Divides ~greedyChildrenRemainingConstraint~ into the ratios greed / sum(greed), creating ~greedyChildrenConstaints~
      - [ ] C: applies each greedyChild it's new  ~greedyChildrenConstaints~ - recursively??? how recursively?? What is the preDescend for ???
      - [ ] ~!!!!!!!Note: The single container wrapped in Greedy, when asked for layoutSize, returns it's layoutSize = constraint~ (the one acquired from ~greedyChildrenConstaints~ ??? *how do we do that* ???
      - [ ] descend greedy - this should give greedy their layoutSize - *how????? we are not using constraints in any way!!! just return fixed on leaf, or calculated on Row and Column, and that does not use layout size!!!! WELL MAYBE THIS CAN WORK: WHEN ASKING GREEDY FOR LAYOUT SIZE, WE SIMPLY ANSWER IT'S NEWLY PROPORTIONALLY CREATED CONSTRAINT SIZE!!!*
      - [ ] POST DESCEND GREEDY - EMPTY:
      - [ ] POST DESCEND ALL
        - [ ] Do what we normally do: ~_postDescend_IfLeaf_Exception_NotLeaf_OffsetImmediateChildrenInMe_ThenSetMySize_Finally_CheckIfMySizeWithinConstraints();~. As we are in Row or Column, this will:
          - [ ] use this row/column layouter to run the one-dim layout of children (greedy and non-greedy, they all have sizes already), use the one-dim layout converted to sizes to apply offsets as normally.
          - [ ] so now all children are offset
          - [ ] *set my size as envelope of children ??? how is that currently done???*
          - [ ] check if my size is within constraints
...
- [X] Resolve why RowLayouter Packing.loose behaves same as Packing.tight !! Reason: Simplicity of single-pass layout. Cannot have Row in Row with nested Row being 'end aligned'.
- [ ] MILESTONE !!!!!! vvvvvvv 
