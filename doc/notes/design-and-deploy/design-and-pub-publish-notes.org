...
* DONE Done
** DONE Add ability to create a Table of Contents to README-source-for-md.org
...
** DONE 2. Easier, superficial charts cleanup and refactoring after Logarithmic scale (but not related)
...
** IN-PROGRESS-NOW 3. ~Constraints and LayoutExpansion: Refactor LayoutExpansion to BoxContainerConstraints and clarify role of BoxContainerConstraints~
...
*** IN-PROGRESS-NOW ~Add BoxContainerConstraints and refactor LayoutExpansion into it~
...
- [-] Code changes:
  ...
  - [ ] Remove members width, height
- [ ] Refactor/rename LayoutExpansion to BoxContainerConstraints
 
** TODO 4. Split Container to Layouter and Painter
See container_base_new.dart *Lessons for Container*

** TODO 5. Container new structural changes (additions not worked in current code yet)
*** TODO Make Containers (Layouters?) a hierarchy and add fields, towards "Everything is a container"

**** NOTE ~Notes ON HIERARCHY OF CONTAINERS~
...
For the Container deep refactoring:
...
**** TODO How do we want the Containers lifecycle eventually?
...
**** TODO Add members and methods to Container class: parent, children, traverse(), flatten(), parentSandbox (maybe this in Layouter)
...
- [ ] Layout related stuff
  ...
  - [ ] implement default method ~layout(BoxContainerConstraints)~ - see [[Layouter general function]]
...
**** TODO Ensure code sets up the hierarchy correctly on all existing Containers,  
***** Study use of GridLinesContainer, XContainer etc, their membership 
***** research which containers hold on parents and children
** TODO 6. Container: Refactorings to use Container new structure from item 5
*** DONE DONE in Container
...
** IN-PROGRESS 7. Layouter algorithm: define and implement
...
*** IN-PROGRESS-NOW Core of Layouter algorithm 
...
#+BEGIN_SRC python
  def layout(Constraint boxContainerConstraintsFromParent):     # 1. boxContainerConstraintss go down (as this is called in parent context)
    if this is leaf:
      this.layoutSize = calculate own layoutSize (may be e.g. size of rectangle if this is a rectangle) if not withit BoxContainerConstraints, exception???
...
#+begin_src dart
  ...
  Offset positionInParent; // result of step 23
  
  List<Container> useExistingChildrenOrCreateChildren();

  Offset findChildPositionIn(childIndex, List<ContainerSize> childrenContainerSizes);
  ...
  void step12AskChildrenToSetDesiredSizes() {
    for (child in children) {
      // todo-00-last
  ...
  void step21SetConstraintsOnChildrenFromDesiredSizes() { /* todo-00-last */ };

  /// Final sizes determination
  void step22AskChildrenToSetFinalSizes() { /* todo-00-last */ };

  /// Final layout step
  void step23PositionChildrenInMeBasedOnFinalSizesAndMyLayoutPropertiesAlsoSetMySize() { /* todo-00-last */ };

  // Final apply parent offset
  void step24ApplyParentOffsetsOnChildren() { /* todo-00-last */ };


  /// Layout means to establish size of this component and position it in parent.
  ...
  Size layout(Constraints constraints) {
    ...
    _children = useExistingChildrenOrCreateChildren(); // return _children by default

    // Sizes from Constraints Pass 1
    step11SetConstraintsOnChildrenFromMy(constraints);
    step12AskChildrenToSetDesiredSizes();

    // Sizes from Constraints Pass 2
    step21SetConstraintsOnChildrenFromDesiredSizes();
    step22AskChildrenToSetFinalSizes();

    // Layout children within me 
    step23PositionChildrenInMeBasedOnFinalSizesAndMyLayoutPropertiesAlsoSetMySize();
    // ^^ This is the core of layout. Maybe this comes from a mixin. Needs lots of helper methods.
    //    Helpers for Row container:
    //      PositionSizes


    // Apply offset from _position
  ...
  class PositionSizes {
    ...
    List<Offset> positionSizesAccordingToAlign();
