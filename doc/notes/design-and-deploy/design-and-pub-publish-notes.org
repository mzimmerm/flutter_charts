...
*** IN-PROGRESS-NOW KEEP : BoxContainer setup parent/child hierarchy in existing containers ~EXCEPT Presenter - VerticalBarPresenter, LineAndHotspotPresenter~
...
- [X] Add  class ~ChartAreaContainerUsingManualLayout extends BoxContainerUsingManualLayout~ and *extend from it all classes that extend ChartAreaContainer EXCEPT LegendContainer*
...
- [ ] Try to get rid of yContainerFirst
- [ ] ~layout~ code: a) Add 'List children' b) after creating each child,  add to children, c) at the end, set children on self. Keep the original members for children, clean that up later. 
  - [ ] ChartRootContainer
  - [ ] ChartAreaContainer
  - [ ] YContainer
  - [ ] XContainer
...
*** IN-PROGRESS KEEP : Old layout and code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~post_NotLeaf_PositionChildren~,  but positioning code is still old, manual, copied from old code!!!
