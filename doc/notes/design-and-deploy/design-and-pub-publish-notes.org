...
*** IN-PROGRESS-NOW Layouter algorithm 
...
*** IN-PROGRESS-NOW Layouter algorith #2

Core ideas

- [ ] BoxContainer 
  - [ ] add field isRoot, set to false
- [ ] RootBoxContainer, set isRoot to true, add abstract rootLayout(Constraints)
- [ ] Rename ChartTopContainer to ChartRootContainer
- [ ] ChartRootContainer extends RootBoxContainer
- [ ] Add to BoxConstainer, field _sandbox, intended to be changed by the owner (not parent)
- [ ] BoxContainerSandbox new class
  - fields childrenGreedyInMainAxisDirection, childrenGreedyInCrossAxisDirection List of children with Size infinity in that direction
  - fields childrenInLayoutOrder
  - fields mainAxisDirection, crossAxisDirection, set to none, X, Y, default none. add method isLayout, returns true if any of the axisdirection is set
- [ ] Allow 0, 1 children to be greedy in MainAxisDirection, otherwise exception
- [ ] ~rootLayout(Constraints)~
  - if !root Exception
  - ~step00_Recurse_createChildrenOrUseChildrenFromConstructor()~ // children should NOT need constraints or layoutSize during construction
  ...
  - ~step10_Recurse_CheckForGreedyChildren_And_PlaceGreedyChildLast~
    - sets up childrenGreedyInMainAxisDirection,  childrenGreedyInCrossAxisDirection
    - if exactly 1 child greedy in MainAxisDirection, put it last in childrenInLayoutOrder, otherwise childrenInLayoutOrder=children
  - this.constraints = passedConstraints 
  - ~step20_Recurse_CalculateAndSetChildrenConstraints_FromMyConstraints()~ // layout specific. only children changed, then next method. Default sets same constraints
    - // node-pre-descend
    - // node-descend  
    - for (child in children)
      - // child-pre-descend
      ...
      - // child-post-descend
    - // node-post-descend     
  - ~step30_Recurse_AskChildrenToSetLayoutSizesWithinTheirConstraints_NodePostDescendOffsetChildrenInMe()~
    - for (child in children)
...
*** IN-PROGRESS-NOW ~AxisDirectionBoxLayouter~, base class for Column and Row layouter
