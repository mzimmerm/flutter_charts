...
** *README.org*: add changes, and *make sure image links point to this link: ~file:/doc/readme_images~, for example:* [ [file:doc/readme_images/README.org_IMGID.png] ]
...
- [ ] Change the links in org file by search-and-replace the string =file:= to =file:doc/readme_images/=
  - [ ] in the org file, the result should look like =file:doc/readme_images/README.org_iterative-layout-step-4.png=
...
1. Ensure README.org file which has the "Table of contents" heading marked as :TOC:noexport: (e.d. TAble of contents :TOC:noexport:) - this causes the TOC update on every save.
   - The :noexport: is currently needed , otherwise error Unable to resolve link: "link-converted-from-first-heading"
2. *README.org*: When ready to export to MD, evaluate once the code in header ~Examples with code~. Follow instructions in header ~Internal notes for exporting this document~ in README.org.
3. *README.org*: Export org to md: ~C-c C-e m m~ in the org file to create the generated md file
4. *README.md*: Delete the TOC generated by the org -> md export, the whole section , including the header named "# Table of Contents" (still needed with :TOC:noexport:)
5. *README.md*: Delete the <a id="YYYY"></a> generated by org conversion: ~delete-matching-lines ^<a id=~
6. *README.md*: Generate md-native TOC:
7. Cursor on top
8. ~M-x: markdown-toc-generate-toc~
9. *README.md*: *NOT NEEDED in 2021-12* Fix image links in the README.md - links must be converted from the first version to the second: 
  #+BEGIN_SRC markdown
