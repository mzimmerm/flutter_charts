...
7. [ ] Create VerticalAxisContainer, has factory to return YContainer wrapped as for ChartSeriesOrientation.column, XContainer otherwise. Generative constructo is private
8. [ ] Pull out Padder AND SizerLayouter from YContainer and DataContainer, and add it as in-place wrappers in ChartRootContainer to dataContainer and vertAxisContainer
9. [ ] Pull out SizerLayouter from XContainer,  and add it as in-place wrappers in ChartRootContainer to dataContainer and horizAxisContainer
10. [ ] Start using Transposing in XContainer, YContainer
11. [ ] Make sure it works in column orientation - Run all tests
12. [ ] NOW THE TRANSPOSE SHOULD WORK IN FULL in column orientation, WITH NO CHANGES FROM Row -> Transposing.Row in code
13. [ ] Run all tests in column orientation
14. [ ] How is row orientation?? Check it out.
15. [ ] Code changes start using Transposing instead of Regular ALMOST EVERYWHERE - FIRST ASK : WHERE NOT?
16. [ ] Change  Row -> Transposing.Row and  ExternalTicksRow -> TransposingExternalTicks.Row in code, but NOT TransposingSizer 
17. [ ] More steps here, or should all work for both column and row???
18. [ ] Add tests for PointOffset lextr method
19. [ ] Replace  PointOffset lextr method and use with proper 2d transforms thought through in Terra Cotta
