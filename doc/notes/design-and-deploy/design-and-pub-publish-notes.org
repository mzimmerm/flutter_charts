...
3. [X] Move ~test/src/example/test_main.dart~ to ~test/src/test_main.dart~ 
4. [X] Look for ~example/lib/main.dart~ AND ~example/lib/test_main.dart~ and change to ~test/src/test_main.dart~ (in comments to  ~lib/test/src/test_main.dart~)
5. [X] Look for ~test_main.dart~ in comments and see if they are correct.
6. [ ] Use the above in test and integration_test instead of example/lib/src/main.dart
7. [X] Change all run configurations to point to the above 
8. [ ] Move example/lib/src/main.dart to lib/test/src/test_main.dart
9. [ ] Repeat item 1.
10. [ ] ~LegendContainer~: In ~lib/test/src/~ create directory ~chart~ and a structure that allows to add test-specific ~LegendContainer~ and perhaps other containters. Refactor/copy file with  ~LegendContainer~ to that directory. Now make following changes:
...
11. [ ] Equivalent of 4. for ~LegendItemContainer~
