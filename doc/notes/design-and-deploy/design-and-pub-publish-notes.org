...
Analyzes of current code (this includes the new layout BUT active use of 'isShown' in new layout is ONLY in ~LegendContainer~ *constructor*, which calls ~applyParentOrderedSkip~:
  ...
  - [X] The options are used in coded_layout examples to convert to no-op all of:
    ...
    - [X] ~applyParentOffset~
    - [X] ~paint~
  - [ ] todo-next: There is ONE *auto_layout* container that sets parentOrderedSkip:
    - [ ] ~LegendContainer constructor~, there is this code:
      #+begin_src java
      ...
      #+end_src
  - [ ] ~BoxLayouter._orderedSkip~ is a field which todo-next
...
- [-] Go over all todo-00- comments and implement. Changes start with:
  ...
  - [ ] Simplification and correcting around - todo-00-now-progress-testing
    #+begin_src java
    ...
    #+end_src
    - [ ] craft above according in each class to contain code suitable to situation
- [ ] Test, validate, commit
- [ ] Go over remaining todo-00- comments, implement
- [ ] Test, validate, commit

  

When done todo-00-next

- [ ] add auto_layout example as in coded_layout which skips labels, axis, grid
...
*** IN-PROGRESS-NOW Review these classes and document somewhere
...
*** TODO-NEXT Current design of LayoutableBox, BoxLayouter, BoxContainer, BoxContainerHierarchy and possible changes
...
class ~BoxLayouter~ extends ~LayoutableBox~ *AND* class ~BoxContainer~ extends ~LayoutableBox~??? *that way BoxLayouter contains all ~LayoutableBox~ implementation AND ~BoxContainer~ also contains ~LayoutableBox~ implementation* ???
