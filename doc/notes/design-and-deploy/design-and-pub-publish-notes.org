...
- [X] Add ability in LegendContainer to create example with specific layouts of ~LegendContainer~
  ...
  - [X] Add 3 examples (only use LineChart) which overrides creation of ~LegendContainer~
    ...
    - [X] Run all tests
    - [X] commit and push
- [-] *Add proper layouter and container hierarchy for offsetting and non-offsetting layouters. In the end, there should be nothing related to ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~ in BoxLayouter or BoxContainer*
  ...
  - [X] Make ~Row and ColumnLayouter~ extend ~RollingOffsettingBoxLayouter~
    ...
    - [X] Move ~mainAxisLayoutProperties and many friends (which? Column and Row, Packing, Lineup)~ from BoxLayouter to ~RollingOffsetBoxLayouter~
