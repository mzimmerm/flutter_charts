#+OPTIONS: toc:3
#+TODO: TODO TODO-NOW TODO-NEXT IN-PROGRESS IN-PROGRESS-NOW LATER LATER-BUT-REVIEW-NEXT EVEN-LATER HOLD DONE-THIS-ITERATION DONE NOTE NOTE-RUNNING


* TODO-NOW Go over this first if not empty

** TODO Warning after upgrade flutter
...
Upgrading gradle.properties
Warning: Flutter support for your project's Gradle version (8.11.0) will soon be dropped. Please upgrade your Gradle version to a version of at least 8.14.0 soon.
Alternatively, use the flag "--android-skip-build-dependency-validation" to bypass this check.

Potential fix: Your project's gradle version is typically defined in the gradle wrapper file. By default, this can be found at /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/gradle/wrapper/gradle-wrapper.properties. 
...
Potential fix: Your project's AGP version is typically defined in the plugins block of the `settings.gradle` file (/home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/settings.gradle), by a plugin with the id of com.android.application. 
If you don't see a plugins block, your project was likely created with an older template version. In this case it is most likely defined in the top-level build.gradle file (/home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/build.gradle) by the following line in the dependencies block of the buildscript: "classpath 'com.android.tools.build:gradle:<version>'".

Potential fix: Your project's KGP version is typically defined in the plugins block of the `settings.gradle` file (/home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/settings.gradle), by a plugin with the id of org.jetbrains.kotlin.android. 
If you don't see a plugins block, your project was likely created with an older template version, in which case it is most likely defined in the top-level build.gradle file (/home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/build.gradle) by the ext.kotlin_version property.

WARNING: Your Android app project: app located at: /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/android/app/build.gradle
...
1. Edited flutter_charts/android/settings.gradle, latest version here:



2. Had to update Intellij, as com.android.application needs update

3. Also had to change gradle-wrapper.properties


* NOTE-RUNNING Current versions of software, current working setup, current test commands

** 2026-05-08 Test command

~tool/test/run_all_tests.sh --auto_layout="chartsGalleryNew layoutsGalleryNew" --coded_layout="oldFailingInNew" --create_screenshots=true~

** For flutter_charts project 2025-10-17

*** Gradle: 2025-10-17: 8.11

mzimmermann@home-server:~/dev/my-projects-source/public-on-github/flutter_charts> ./android/gradlew --version
...
Launcher JVM:  1.8.0_462 (IcedTea 25.462-b08)
Daemon JVM:    /usr/lib64/jvm/java-1.8.0-openjdk-1.8.0/jre (no JDK specified, using current Java home)
OS:            Linux 6.17.1-1.0.4.sr20251001-default amd64



*** AGP (Android Gradle plugin): 2025-10-17: 8.7.3

AGP version is the string 8.7.3 in ~flutter_charts/android/gradle.properties~. NOTE that this section is for project settings defined by Kotlin DSL which I now use throughout (NOT groovy DSL)
...
*** Java: 2025-10-17: 21 (jbr-21)

Go to project structure - SDK - java: jbr-21
...
4. *README.md*:
   ...
   + *Fix image links in the README.md* - links must be converted from the first version to the second: 
    ...
    #+END_SRC
   + The above is achieved with emacs command:
...
** ~git checkout master; git fetch; git pull; git merge flutter_charts~
    ...
    - yScaler is created from this code
      #+begin_src dart
        ...
        YScalerAndLabelFormatter makeLabelsFromDataOnScale({
          ...
          //  Allow start Y axis from non-zero
          ...
          // print( " ################ makeLabelsFromData: For ###_values=$_values found ###labeValues=${labels} and ###axisInterval= ${from} to ${to} ");

          var yScaler = YScalerAndLabelFormatter(
            axisInterval: Interval(from, to),
...
** NOTE 1. Documentation: Containers and Container-like classes overview

todo-01: clarify documentation of layouter vs container

generally:

- Layouter extends Container
- Layouter is a Container and nothing more (so maybe Layouters should be removed)
- Layouter sets ~isLeaf~ to false to express it should have children.


*** Auto-Layout

**** BoxContainer and extensions

- ~BoxContainer extends BoxContainerHierarchy with BoxLayouter implements LayoutableBox, Keyed, UniqueKeyedObjectsManager~
  - members or set from constructor
    - ~key~ (implement Keyed)
    - ~children~ (to set BoxContainerHierarchy.__children)
    - ~constraintsWeight~
    - ~sandbox~
    
  - extended by ~PositioningBoxContainer extends BoxContainer~
    - allowed to position children; to achieve that, is passed required ~constraintsWeight~ which is used to position children
  - extended by ~PositioningBoxLayouter extends PositioningBoxContainer~
    - ONLY sets ~isLeaf~ to false
      
  - extended by ~NonPositioningBoxLayouter extends BoxContainer~
    - sets ~isLeaf~ to false

    - Overrides ~layout_Post_NotLeaf_PositionChildren~ which does NOT change children positions

    - Overrides ~_layout_Post_NotLeaf_OffsetChildren~ with no-op so chidren positions are  NOT changed





todo-01 document further extensions: ANYTHINGContainer, concentrate on DataContainer

*** COMMENT Coded Layout

- ~ChartRootContainerCL~
  - extended by ~BarChartRootContainerCL~ and ~LineChartRootContainerCL~ extends ~ChartRootContainerCL~ implements ~ChartRootContainer~
  - members
    #+begin_src java
      @override
    ...
    todo-01 document the contained containers.
...
#+begin_src dart
  ...
  void step11SetConstraintsOnChildrenFromMy(Constraints myConstraints) { /* later */ };
  ...
  void step21DistributeConstraintsOnChildrenBasedOnDesiredSizes() { /* later */ };

  /// Final sizes determination
  void step22AskChildrenToSetFinalSizesWithinTheirConstraints() { /* later */ };

  /// Final layout step
  void step23OffsetChildrenInMeBasedOnFinalSizesAndMyLayoutPropertiesAlsoSetMySize() { /* later */ };

  // Final apply parent offset
  void step24ApplyParentOffsetsOnChildren() { /* later */ };
...
*** DONE KEEP - contains valuable algorithmic notes : ~layout()~ code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~newCoreLayout~,  but positioning code is still old, manual, copied from old code!!!
...
*** DONE Go over coded_layout classes and ensure they are named CL and NOT used outside coded_layout

- Use IntelliJ regex replace examples:
  - ([^a-zA-Z])(PointPresenter)([^a-zA-Z]) => $1$2OCL$3
  
- [X] All classes in  coded_layout make CL. Concentrate on all 'presenter.dart' files
  - [X] PointPresenter        => PointPresenterOCL
  - [X] PointPresenterCreator => PointPresenterCreatorOCL
  - [X] PointPresentersColumns => PointPresentersColumnsOCL
  - [X] PointPresentersColumn => PointPresentersColumnOCL
  - [X] PixelRangeProvider     => PixelRangeProviderOCL
  - [X] StackableValuePoint    => StackableValuePointOCL
  - [X] PointsColumns          => PointsColumnsOCL
  - [X] PointsColumn           => PointsColumnOCL
- [X] Ensure no CL class used outside coded_layout, maybe with minor well defined exceptions
...
- [X] Achieve in BoxContainer constructor:
  ...
  - [X] Either way, ~ensureKeyedMembersHaveUniqueKeys()~ is called just after, to ensure the passed [children] or the single child from [buildContainerOrSelf] has a unique key
...
~flutter drive --dart-define=EXAMPLE_DESCRIPTORS='absoluteMinimumNew' --driver=test_driver/integration_test.dart  --target=integration_test/screenshot_create_test_new.dart~

followed by

~flutter test --dart-define=EXAMPLE_DESCRIPTORS='absoluteMinimumNew' test/screenshot_validate_test_new.dart~
...
- [X] Add (first only) TransposingInputValuesGrid, which has lines layed out using ExternalTicksColumn. Must use same layout as labels.
...
** DONE 17. Change example and test directory structure around main.dart.  Move example/lib/src/main.dart to lib/test/src/test_main.dart; create standard and alternative layouts
...
*** DONE Script to rename images to standard names


The script below renames images to use a unique descriptive names. One-time only but KEEP.

#+name: rename-images-in-integration-test
#+begin_src bash :results raw
  cd /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/integration_test/screenshots_expected/
...
mv ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart.png                                                                                               ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_oldManualLayouter.png
mv ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart.png                                                                                              ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_oldManualLayouter.png
...
mv ex52LogarithmicScale_barChart.png                                                                                            ex52LogarithmicScale_barChart_column_stacked_oldManualLayouter.png
mv ex52LogarithmicScale_lineChart.png                                                                                           ex52LogarithmicScale_lineChart_column_nonStacked_oldManualLayouter.png
...
*** DONE Change the example and test directory structure
...
2. [X] Move example/lib/src/main.dart to ~lib/test/src/example/test_main.dart~
3. [X] Move ~test/src/example/test_main.dart~ to ~lib/test/src/test_main.dart~ 
4. [X] Look for ~example/lib/main.dart~ AND ~example/lib/test_main.dart~ and change to ~lib/test/src/test_main.dart~ (in comments to  ~lib/test/src/test_main.dart~)
5. [X] Look for ~test_main.dart~ in comments and see if they are correct.
6. [X] Use the above in test and integration_test instead of example/lib/src/main.dart
7. [X] Change all run configurations to point to the above


*** DONE Allow extendable Legend Layout

Allow tests and clients to extend LegendContainer and layout legend items freely.

There are 2 methods: use test-defined or client-defined enums in options.dart.

- For tests, we SHOULD use enums in options.dart to support clear naming in tests.
- If a client wishes to NOT add a layout enum, they can follow the test ... *legend_container.dart* but not use any new enums - simply setup any layout they wish..


The steps to do here:

- [X] Options should have limited Legend layouts, controlled by ~LegendOptions.legendAndItemLayoutEnum~, which clients can use without adding extensions of LegendContainer
  - [X] Src *options.dart*  
    - legendIsRowStartTightItemIsRowStartTightDefault
    - legendIsWrappingRowItemIsRowStartTight
  - [X] Test *options.dart*  
    - legendIsColumnStartLooseItemIsRowStartLoose
    - legendIsColumnStartTightItemIsRowStartTight
    - legendIsRowCenterLooseItemIsRowEndLoose
    - legendIsRowStartTightItemIsRowStartTightSecondGreedy
    - legendIsRowStartTightItemIsRowStartTightItemChildrenPadded
    - legendIsRowStartTightItemIsRowStartTightItemChildrenAligned
- [X] Simplify chart ..legend_container.dart -> LegendContainer to only use the 2 default ~LegendOptions.legendAndItemLayoutEnum~ 
- [X] Equivalent of the above for ~LegendItemContainer~
   

*** DONE Clarify difference _make , _create



** DONE 17.1. Separate integration test images between 'oldManualLayouter' and 'newAutoLayouter'. Identify charts that do NOT work in auto_layout.

#+begin_src bash
  cd /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/integration_test/screenshots_tested/
  ls -lt
#+end_src

#+name: images-tested-before-2025-10-21
|                                                                                                                               |                                                                                                                                                                                                                                                                                                                                                         |                                                                                                                               | Visual comparison with    | If NOT OK,                                                                                                                                                            |
|                                                                                                                               |                                                                                                                                                                                                                                                                                                                                                         |                                                                                                                               | old image OK, move to new | describe issue in new                                                                                                                                                 |
|-------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------+---------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ex900ErrorFixUserDataAllZero_lineChart_column_nonStacked_newAutoLayouter.png                                                  | okular integration_test/screenshots_expected/ex900ErrorFixUserDataAllZero_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex900ErrorFixUserDataAllZero_lineChart_column_nonStacked_newAutoLayouter.png                                                                                                   | ex900ErrorFixUserDataAllZero_lineChart_column_nonStacked_oldManualLayouter.png                                                | NO                        | [err  0] in new, chart not created, just white                                                                                                                        |
| ex76AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenAligned_barChart_column_stacked_newAutoLayouter.png    | okular integration_test/screenshots_expected/ex76AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenAligned_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex76AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenAligned_barChart_column_stacked_newAutoLayouter.png       | ex76AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenAligned_barChart_column_stacked_newAutoLayouter.png    | OK                        | -                                                                                                                                                                     |
| ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_lineChart_column_nonStacked_newAutoLayouter.png | okular integration_test/screenshots_expected/ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_lineChart_column_nonStacked_newAutoLayouter.png | ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_lineChart_column_nonStacked_newAutoLayouter.png | OK                        | -                                                                                                                                                                     |
| ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_barChart_column_stacked_newAutoLayouter.png     | okular integration_test/screenshots_expected/ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_barChart_column_stacked_newAutoLayouter.png         | ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_barChart_column_stacked_newAutoLayouter.png     | OK                        | -                                                                                                                                                                     |
| ex74AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightSecondGreedy_barChart_column_stacked_newAutoLayouter.png           | okular integration_test/screenshots_expected/ex74AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightSecondGreedy_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex74AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightSecondGreedy_barChart_column_stacked_newAutoLayouter.png                     | ex74AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightSecondGreedy_barChart_column_stacked_newAutoLayouter.png           | OK                        | -                                                                                                                                                                     |
| ex73AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                       | okular integration_test/screenshots_expected/ex73AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTight_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex73AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                                             | ex73AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                       | OK                        | [warn 1] in new, legend is centered instead of left start. Probably fine                                                                                              |
| ex72AnimalsBySeasonLegendIsRowCenterLooseItemIsRowEndLoose_barChart_column_stacked_newAutoLayouter.png                        | okular integration_test/screenshots_expected/ex72AnimalsBySeasonLegendIsRowCenterLooseItemIsRowEndLoose_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex72AnimalsBySeasonLegendIsRowCenterLooseItemIsRowEndLoose_barChart_column_stacked_newAutoLayouter.png                                               | ex72AnimalsBySeasonLegendIsRowCenterLooseItemIsRowEndLoose_barChart_column_stacked_oldManualLayouter.png                      | NO                        | [err  1] in new, overflow which in old is only in legend, spills to the whole chart                                                                                   |
| ex71AnimalsBySeasonLegendIsColumnStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                    | okular integration_test/screenshots_expected/ex71AnimalsBySeasonLegendIsColumnStartTightItemIsRowStartTight_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex71AnimalsBySeasonLegendIsColumnStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                                       | ex71AnimalsBySeasonLegendIsColumnStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                    | OK                        | [warn 1]                                                                                                                                                              |
| ex70AnimalsBySeasonLegendIsColumnStartLooseItemIsRowStartLoose_barChart_column_stacked_newAutoLayouter.png                    | okular integration_test/screenshots_expected/ex70AnimalsBySeasonLegendIsColumnStartLooseItemIsRowStartLoose_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex70AnimalsBySeasonLegendIsColumnStartLooseItemIsRowStartLoose_barChart_column_stacked_newAutoLayouter.png                                       | ex70AnimalsBySeasonLegendIsColumnStartLooseItemIsRowStartLoose_barChart_column_stacked_newAutoLayouter.png                    | OK                        | [warn 1]                                                                                                                                                              |
| ex60LabelsIteration4_barChart_column_stacked_newAutoLayouter.png                                                              | okular integration_test/screenshots_expected/ex60LabelsIteration4_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex60LabelsIteration4_barChart_column_stacked_newAutoLayouter.png                                                                                                                           | ex60LabelsIteration4_barChart_column_stacked_oldManualLayouter.png                                                            | NO                        | [err  2] in new labels not tilted or skipped when pressed , also [err  1], also [ warn 1]                                                                             |
| ex60LabelsIteration3_barChart_column_stacked_newAutoLayouter.png                                                              | okular integration_test/screenshots_expected/ex60LabelsIteration3_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex60LabelsIteration3_barChart_column_stacked_newAutoLayouter.png                                                                                                                           | ex60LabelsIteration3_barChart_column_stacked_oldManualLayouter.png                                                            | NO                        | [err  2]  also [ warn 1 ]                                                                                                                                             |
| ex60LabelsIteration2_barChart_column_stacked_newAutoLayouter.png                                                              | okular integration_test/screenshots_expected/ex60LabelsIteration2_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex60LabelsIteration2_barChart_column_stacked_newAutoLayouter.png                                                                                                                           | ex60LabelsIteration2_barChart_column_stacked_oldManualLayouter.png                                                            | NO                        | [err  2]  also [ warn 1 ]                                                                                                                                             |
| ex60LabelsIteration1_barChart_column_stacked_newAutoLayouter.png                                                              | okular integration_test/screenshots_expected/ex60LabelsIteration1_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex60LabelsIteration1_barChart_column_stacked_newAutoLayouter.png                                                                                                                           | ex60LabelsIteration1_barChart_column_stacked_newAutoLayouter.png                                                              | OK                        | [warn 1]                                                                                                                                                              |
| ex52LogarithmicScale_barChart_column_stacked_newAutoLayouter.png                                               | okular integration_test/screenshots_expected/ex52LogarithmicScale_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex52LogarithmicScale_barChart_column_stacked_newAutoLayouter.png                                                                                             | ex52LogarithmicScale_barChart_column_stacked_newAutoLayouter.png                                               | OK                        | [warn 2] in new, long label 1000000000 is wrapped, maybe desider, also [warn 1]                                                                                       |
| ex52LogarithmicScale_lineChart_column_nonStacked_newAutoLayouter.png                                           | okular integration_test/screenshots_expected/ex52LogarithmicScale_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex52LogarithmicScale_lineChart_column_nonStacked_newAutoLayouter.png                                                                                     | ex52LogarithmicScale_lineChart_column_nonStacked_newAutoLayouter.png                                           | OK                        | [warn 1] also [warn 2]                                                                                                                                                |
| ex50StocksWithNegativesWithUserColors_barChart_column_stacked_newAutoLayouter.png                                             | okular integration_test/screenshots_expected/ex50StocksWithNegativesWithUserColors_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex50StocksWithNegativesWithUserColors_barChart_column_stacked_newAutoLayouter.png                                                                                         | ex50StocksWithNegativesWithUserColors_barChart_column_stacked_newAutoLayouter.png                                             | OK                        | [warn 1]                                                                                                                                                              |
| ex40LanguagesWithYOrdinalUserLabelsAndUserColors_lineChart_column_nonStacked_newAutoLayouter.png                              | okular integration_test/screenshots_expected/ex40LanguagesWithYOrdinalUserLabelsAndUserColors_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex40LanguagesWithYOrdinalUserLabelsAndUserColors_lineChart_column_nonStacked_newAutoLayouter.png                                                           | ex40LanguagesWithYOrdinalUserLabelsAndUserColors_lineChart_column_nonStacked_oldManualLayouter.png                            | NO                        | [err  0]                                                                                                                                                              |
| ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_newAutoLayouter.png                                                  | okular integration_test/screenshots_expected/ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_newAutoLayouter.png                                                                                                   | ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_oldManualLayouter.png                                                | NO                        | [err  3] in new, shows labels, grid, and axes, but it should not. NOte: in old as expected, the non dislayed is NOT layed outin new as expected: legend does not show |
| ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_newAutoLayouter.png                                              | okular integration_test/screenshots_expected/ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_newAutoLayouter.png                                                                                           | ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_oldManualLayouter.png                                            | NO                        | [err  3]                                                                                                                                                              |
| ex34OptionsDefiningUserTextStyleOnLabels_lineChart_column_nonStacked_newAutoLayouter.png                                      | okular integration_test/screenshots_expected/ex34OptionsDefiningUserTextStyleOnLabels_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex34OptionsDefiningUserTextStyleOnLabels_lineChart_column_nonStacked_newAutoLayouter.png                                                                           | ex34OptionsDefiningUserTextStyleOnLabels_lineChart_column_nonStacked_newAutoLayouter.png                                      | OK                        | [warn 1]                                                                                                                                                              |
| ex33AllNegativeYsYAxisEndsBelow0_lineChart_column_nonStacked_newAutoLayouter.png                                              | okular integration_test/screenshots_expected/ex33AllNegativeYsYAxisEndsBelow0_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex33AllNegativeYsYAxisEndsBelow0_lineChart_column_nonStacked_newAutoLayouter.png                                                                                           | ex33AllNegativeYsYAxisEndsBelow0_lineChart_column_nonStacked_oldManualLayouter.png                                            | NO                        | [err  0] maybe due to negative values on line chart?                                                                                                                  |
| ex32AllPositiveYsYAxisStartsAbove0_barChart_column_stacked_newAutoLayouter.png                                                | okular integration_test/screenshots_expected/ex32AllPositiveYsYAxisStartsAbove0_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex32AllPositiveYsYAxisStartsAbove0_barChart_column_stacked_newAutoLayouter.png                                                                                               | ex32AllPositiveYsYAxisStartsAbove0_barChart_column_stacked_newAutoLayouter.png                                                | OK                        | [warn 1]                                                                                                                                                              |
| ex32AllPositiveYsYAxisStartsAbove0_lineChart_column_nonStacked_newAutoLayouter.png                                            | okular integration_test/screenshots_expected/ex32AllPositiveYsYAxisStartsAbove0_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex32AllPositiveYsYAxisStartsAbove0_lineChart_column_nonStacked_newAutoLayouter.png                                                                                       | ex32AllPositiveYsYAxisStartsAbove0_lineChart_column_nonStacked_oldManualLayouter.png                                          | NO                        | [err  0] maybe due to linear chart not starting at positive value?                                                                                                    |
| ex31SomeNegativeValues_barChart_column_stacked_newAutoLayouter.png                                                            | okular integration_test/screenshots_expected/ex31SomeNegativeValues_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex31SomeNegativeValues_barChart_column_stacked_newAutoLayouter.png                                                                                                                       | ex31SomeNegativeValues_barChart_column_stacked_newAutoLayouter.png                                                            | OK                        | [warn 1]                                                                                                                                                              |
| ex31SomeNegativeValues_lineChart_column_nonStacked_newAutoLayouter.png                                                        | okular integration_test/screenshots_expected/ex31SomeNegativeValues_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex31SomeNegativeValues_lineChart_column_nonStacked_newAutoLayouter.png                                                                                                               | ex31SomeNegativeValues_lineChart_column_nonStacked_newAutoLayouter.png                                                        | OK                        | [warn 1]                                                                                                                                                              |
| ex30AnimalsBySeasonWithLabelLayoutStrategy_barChart_column_stacked_newAutoLayouter.png                                        | okular integration_test/screenshots_expected/ex30AnimalsBySeasonWithLabelLayoutStrategy_barChart_column_stacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex30AnimalsBySeasonWithLabelLayoutStrategy_barChart_column_stacked_newAutoLayouter.png                                                                               | ex30AnimalsBySeasonWithLabelLayoutStrategy_barChart_column_stacked_newAutoLayouter.png                                        | OK                        | [warn 1]                                                                                                                                                              |
| ex30AnimalsBySeasonWithLabelLayoutStrategy_lineChart_column_nonStacked_newAutoLayouter.png                                    | okular integration_test/screenshots_expected/ex30AnimalsBySeasonWithLabelLayoutStrategy_lineChart_column_nonStacked_oldManualLayouter.png & okular integration_test/screenshots_tested/ex30AnimalsBySeasonWithLabelLayoutStrategy_lineChart_column_nonStacked_newAutoLayouter.png                                                                       | ex30AnimalsBySeasonWithLabelLayoutStrategy_lineChart_column_nonStacked_newAutoLayouter.png                                    | OK                        | [warn 1]                                                                                                                                                              |
|                                                                                                                               |                                                                                                                                                                                                                                                                                                                                                         |                                                                                                                               |                           |                                                                                                                                                                       |

Succesful Charts in new auto_layout if failing, using old coded_layout:

ex900ErrorFixUserDataAllZero_lineChart_column_nonStacked_oldManualLayouter.png                                                
ex76AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenAligned_barChart_column_stacked_newAutoLayouter.png    
...
ex74AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightSecondGreedy_barChart_column_stacked_newAutoLayouter.png           
ex73AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                       
ex72AnimalsBySeasonLegendIsRowCenterLooseItemIsRowEndLoose_barChart_column_stacked_oldManualLayouter.png                      
ex71AnimalsBySeasonLegendIsColumnStartTightItemIsRowStartTight_barChart_column_stacked_newAutoLayouter.png                    
ex70AnimalsBySeasonLegendIsColumnStartLooseItemIsRowStartLoose_barChart_column_stacked_newAutoLayouter.png                    
...
ex60LabelsIteration1_barChart_column_stacked_newAutoLayouter.png                                                              
ex52LogarithmicScale_barChart_column_stacked_newAutoLayouter.png
ex52LogarithmicScale_lineChart_column_nonStacked_newAutoLayouter.png
ex50StocksWithNegativesWithUserColors_barChart_column_stacked_newAutoLayouter.png                                             
ex40LanguagesWithYOrdinalUserLabelsAndUserColors_lineChart_column_nonStacked_oldManualLayouter.png                            
ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_barChart_column_stacked_oldManualLayouter.png
ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown_lineChart_column_nonStacked_oldManualLayouter.png
ex34OptionsDefiningUserTextStyleOnLabels_lineChart_column_nonStacked_newAutoLayouter.png                                      
ex33AllNegativeYsYAxisEndsBelow0_lineChart_column_nonStacked_oldManualLayouter.png                                            
ex32AllPositiveYsYAxisStartsAbove0_barChart_column_stacked_newAutoLayouter.png                                                
...
ex30AnimalsBySeasonWithLabelLayoutStrategy_barChart_column_stacked_newAutoLayouter.png                                        
ex30AnimalsBySeasonWithLabelLayoutStrategy_lineChart_column_nonStacked_newAutoLayouter.png                                    
                                                                                                                              


** DONE 17.2. Refactor naming around view_model.ChartViewModel and label_model.AxisIntervalTicksAndLabelsDescriptor

- [X] this  ChartViewModel.valuesInterval() is core for ex900 bug. valuesInterval is NOT tight data envelope, BUT it is transformed data envelope potentially extended to 0.0 origin

- [X] rename extendedValuesInterval FUNCTION to findTrfdDataExtendedInterval
- [X] rename view_model. valuesInterval FUNCTION to findTrfdDataTightInterval
- [X] rename dataEnvelope => trfdDataExtendedInterval
- [X] rename dataRange => axisInterval (more descriptive name: fullAxisIntervalAsTrfdData)
- [X] rename transformedLabelValues => trfdLabelNumbers
- [X] rename _generateLabelNumbersForLabelsCountInInterval => _generateLabelNumbersForLabelsCountInInterval
- [X] rename  _generateValuesForLabelsIn => _generatePolySuitableLabelNumbersInInterval
- [X] rename labelsCount => labelsCount
- [X] rename extremeValueWithSign => outerDatumForSign
- [X] rename _transformedValuesMin => _trfdDataMin
- [X] rename _transformedValuesMax => _trfdDataMax
- [X] look at places where 'transformed' is used and rename to trfd
 

Note: Places where 'envelope' is used: currently only for Rectangle and it means tight envelope, keep that.


** DONE 18. Simplify project structure regarding coded_layout: Make coded_layout and new_layout less dependend for testing: Remove SwitchViewModels, switch_view_model.dart, LiveOrTesting use instead test_coded_layou_main.dart in addition to test_main.dart

intellij:
- how to compare files?
- search class ([0-9_A-Za-z]+)[^O]CL
- search class Switch([0-9_A-Za-z]+)


*** DONE Make test script changes and duplicate 2 test files from auto_layout to coded_layout

In general, some libraries or classes in 'testing' are merged back to lib OR create a new library under lib/src/custom

1. [X] move lib/test/src/test/options.dart to lib/src/chart/layout_alternatives
2. [X] move lib/test/src/test/chart/cartesian/container/options.dart to lib/src/chart/layout_alternatives/cartesian/container/options.dart
3. [X] TEST FOR SAME
4. [X] USE 2 TEST APPS : ONE FOR auto_layout, OTHER for coded_layout with the eventual goal of coded_layout gone.
   1) [X] Copy test_main.dart to coded_layout_test_main.dart
   2) [X] Copy integration_test/screenshot_create_test.dart to coded_layout_screenshot_create_test.dart
      1. [X] in the coded_layout version coded_layout_screenshot_create_test.dart, instead of IMPORTING AND CALLING  test_main.dart, call coded_layout_test_main.dart
   3) [X] Modify run-all-tests.sh and related scripts to accept auto_layout and coded_layout options separately
      1. [X]  run-all-tests.sh --auto-layout="ex1 ex2 group1 group2" --coded-layout"ex1 ex3 group3"
         + [X] In processing, of run_all_tests.sh:
           - [X] no change
         + [X]  In processing, of run_screenshots_create_then_validate_integration_test.sh
           - [X] check for --coded-layout and --auto-layout, pick the codedLayoutExampleDescriptors and autoLayoutExampleDescriptors
           - [X] Run flutter drive 2x, separately for coded_layout and auto_layout, passing the appropriate example descriptors.
   4) [X] TEST FOR SAMENESS AND COMMIT
      + [X] Delete the tested files first, to make sure all images are generated
      + [X] run_all_tests.sh --auto-layout="chartsGalleryNew convertedToNew" --coded-layout="oldFailingInNew"
      + [X] COMMIT
   5) [X] NEXT STEP IS INTRODUCE A COPY WITH MINOR CHANGE OF THE TEST FILES FOR CODED_LAYOUT AND AUTO_LAYOUT, SO THAT WE ONLY HAVE TO MAINTAIN ONE VERSION, the other will ba copied with sed edit changes:
      1. [X] In run_screenshots_create_then_validate_integration_test.sh before calling the loop around auto_layout coded_layout:
         * [X] copy test_main.dart to coded_layout_test_main.dart with sed editing DOING NOTHING; LATER THIS WILL MAKE SURE CODED_LAYOUT VERSION IS CALLED
         * [X] Copy screenshot_create_test.dart to coded_layout_screenshot_create_test.dart with sed editing REPLACING
           - import \(.*\)\(test_main.dart' as on_device_test_app;\)
           - import \1coded_layout\2
      2. [X] TEST FOR SAMENESS
      3. [X] COMMIT




*** DONE Remove SwitchViewModels, switch_view_model.dart, LiveOrTesting


~makeChartRootContainer~ is declared in:

1. lib/test/src/switch_view_model/coded_layout/bar/view_model.dart:  BarChartViewModelCL    as: BarChartRootContainer    makeChartRootContainer({})
2. lib/test/src/switch_view_model/coded_layout/line/view_model.dart: LineChartViewModelCL   as: LineChartRootContainer   makeChartRootContainer({})
3. lib/src/switch_view_model/coded_layout/bar/view_model.dart:       BarChartViewModelCLCL  as: BarChartRootContainerCL  makeChartRootContainer({})
4. lib/src/switch_view_model/coded_layout/line/view_model.dart:      LineChartViewModelCLCL as: LineChartRootContainerCL makeChartRootContainer({})
5. lib/src/switch_view_model/auto_layout/bar/view_model.dart:        BarChartViewModelCL    as: BarChartRootContainer    makeChartRootContainer({})
6. lib/src/switch_view_model/auto_layout/line/view_model.dart:       LineChartViewModelCL   as: LineChartRootContainer   makeChartRootContainer({})
7. lib/src/switch_view_model/view_model_cl.dart:                     ChartViewModelCLCL     as: ChartRootContainerCL     makeChartRootContainer({})
8. lib/src/chart/view_model/view_model.dart:                         ChartViewModel         as: ChartRootContainer       makeChartRootContainer({})


BUT there is also:
   lib/src/switch_view_model/view_model.dart:                        ChartViewModelCL!!!! which does NOT have ~makeChartRootContainer~ BUT has the FACTORIES ChartViewModelCL.barChartViewModelFactory return ~BarChartViewModelCLCL~ for *oldLayouter* and return ~BarChartViewModelCL~ for *newLayouter*
...
chart/cartesian          ChartRootContainer            extends ChartAreaContainer
coded_layout/chart       ChartRootContainerCL          extends ChartAreaContainer implements ChartRootContainer

chart/cartesian/bar      BarChartRootContainer         extends ChartRootContainer
chart/cartesian/line     LineChartRootContainer        extends ChartRootContainer

coded_layout/chart/bar   BarChartRootContainerCL       extends ChartRootContainerCL
coded_layout/chart/line  LineChartRootContainerCL      extends ChartRootContainerCL

    NOTES:
    - In Containers, CL means coded_layout

------
9 VIEW_MODELS (plus 1 mock)
------
root                            ChartViewModel               extends Object             CHANGE: 
switch                          ChartViewModelCL             extends ChartViewModel     CHANGE: REMOVE
switch                          ChartViewModelCLCL           extends ChartViewModelCL   CHANGE: EXTEND ChartViewModel
                                                                                        
switch/auto_layout/bar          BarChartViewModelCL          extends ChartViewModelCL   CHANGE: extend ChartViewModel directly
switch/auto_layout/line         LineChartViewModelCL         extends ChartViewModelCL:  CHANGE: extend ChartViewModel directly
                                                                                         
switch/coded_layout/bar         BarChartViewModelCLCL        extends ChartViewModelCLCL CHANGE: REMOVE, REPLACE WITH TEST VERSION        
switch/coded_layout/line        LineChartViewModelCLCL       extends ChartViewModelCLCL CHANGE: REMOVE, REPLACE WITH TEST VERSION           

test/switch/coded_layout/bar    BarChartViewModelCL          extends auto_layout_bar_chart_view_model.BarChartViewModelCL   : CHANGE: LATER MOVE  TO ????
test/switch/coded_layout/line   LineChartViewModelCL         extends auto_layout_line_chart_view_model.LineChartViewModelCL

    NOTES:
    - In view models, CL   means switch                  - WE SHOULD NOT NEED THOSE
    - In view models, CLCL means switch + coded layout   - WE SHOULD ABBREVIATE TO CL


   - [X] replace ViewModelCL with ViewModel case sensitive
   - [X] replace viewModelCL with viewModel case sensitive (none)
   - [X] move everything from switch directory away:
     - [X] move src/switch_view_model/auto_layout/line/view_model.dart => src/chart/view_model/line/line_view_model.dart
     - [X] move src/switch_view_model/coded_layout/line/view_model.dart => src/coded_layout/chart/view_model/line/coded_layout_line_view_model.dart
     - [X] same for bar
     - [X] special care: view_model_cl.dart - where to move?
   - [X] Rename packages 'as switch' such as switch_auto_layout_bar_view_model
   - [X] Search for switch Switch



   
*** DONE 18.1. Ensure test_main.dart copy and replace to coded_layout_test_main.dart is working.

Fix ~function _duplicate_test_files_from_auto_layout_to_coded_layout()~ in ./tool/test/run_screenshots_create_then_validate_integration_test.sh to copy test_main.dart to coded_layout_test_main.dart and replace appropriate strings to convert the auto_layout test to coded_layout test.

These are the differences that should be implemented by the sed

#+begin_example
mzimmermann@home-server:~/dev/my-projects-source/public-on-github/flutter_charts> find . -name "*test_main.dart"
./lib/test/src/test_main.dart
./lib/test/src/coded_layout_test_main.dart
mzimmermann@home-server:~/dev/my-projects-source/public-on-github/flutter_charts> diff ./lib/test/src/test_main.dart ./lib/test/src/coded_layout_test_main.dart
28a29,32
...
*** DONE 18.2 Change Legend class : split into multiple factories; remove dependency on the ex enums

*** DONE 18.3 Add a example ex001 with non-test LegendContainer

- [X] rename ex52AnimalsBySeasonLogarithmicScale to ex52LogarithmicScale
- [X] ex30AnimalsBySeasonWithLabelLayoutStrategy seems to repeat animal data - FIX and validate
- [X] test_main.dart - pull animal data up as default and change examples using them
- [X] find other examples that use animal data
- [X] test and commit
- [X] Add 2 *live* LegendContainer layout factories
  - [X] Center - find current test defauls legendIsRowStartTightItemIsRowStartTightDefault
  - [X] Change name legendIsRowStartTightItemIsRowStartTightDefault to legendIsRowStartTightItemIsRowStartTight
  - [X] Add 
  - [X] WrappingRow (exists: liveLegendIsRowStartTightItemIsRowStartTightDefault) - legend_container.LegendContainer.liveLegendIsRowStartTightItemIsRowStartTightDefault - DEFAULT
- [X] test_main.dart: Add a non-test example ex001AnimalsBySeasonLiveLegendContainer: 
  - [X] add enum to all switches:
    - [X] add to all switches: In TEST LegendContainer, throw exception, it should not reach ex200
  - [X] Use animal data
  - [X] Use live LegendContainer default
  - [X] Comment as example of using live LegendContainer which does NOT use enums internally (as opposed to test LegendContainer) 
  - [X] Add example to chartsGalleryNew (only one, say nonStacked Bar)
  - [X] Add images
  - [X] retest and commit
- 

** DONE ~19. Add ability to ignore BoxContainer during layout and/or paint. USE IT to show chart without axes, and/or labels, and/or legend~

*** DONE Add ability to ignore BoxContainer during layout and/or paint

This todo is also documented in container_layouter_base.dart

Study code around current ParentOrderedSkip . What does it do? It uses various variable names in Options containing 'isShown', which are then used in 'layout', 'applyParentOffset' and 'paint'. If isShown is false, those methods are no-op - they return before they can do anything.

Analyzes of current code (this includes the new layout BUT active use of 'isShown' in new layout is ONLY in ~LegendContainer~ *constructor*, which calls ~applyParentOrderedSkip~:

  - [X] Options set 'isShown' with various naming changes on:
    - [X] LegendOptions.isLegendContainerShown
    - [X] HorizontalAxisContainerOptions.isShown
    - [X] VerticalAxisContainerOptions.isShown
    - [X] VerticalAxisContainerOptions.isInputGridLinesShown
  - [X] The options are used in coded_layout examples to convert to no-op all of:
    - [X] ~layout~
    - [X] ~applyParentOffset~
    - [X] ~paint~
  - [X] There is ONE *auto_layout* container that sets parentOrderedSkip:
    - [X] ~LegendContainer constructor~, there is this code:
      #+begin_src java
        if (!chartViewModel.chartOptions.legendOptions.isLegendContainerShown) {
          applyParentOrderedSkip(this, true); // method defined on mixin BoxLayouter
          // BoxLayouter.applyParentOrderedSkip controls the BoxLayouter._orderedSkip
        }
      #+end_src
  - [X] ~BoxLayouter._orderedSkip~ is a field which manages visibility


Summary of changes which implement ParentOrderedSkip in the new auto_layout:
  
    
- [X] add class ParentOrderedSkip beside BoxLayouter mixin
  - [X] bool isSkipLayout
  - [X] bool isSkipPaint
  - [X] private generative constructor
  - [X] const static skipPaint
  - [X] const static skipLayoutAndPaint
  - [X] const static skipNone (DEFAULT)
  - [X] 
- [X] on mixin BoxLayouter, add member _orderedSkip, and public get orderedSkip
  - [X] default to skipNone
  - [X] not final, we want to be able layout code to force skip if needed
- [X] review options related to skipping. THE options VARIABLE IS CALLED 'isShown' in general :
  - [X] LegendOptions.isLegendContainerShown
  - [X] HorizontalAxisContainerOptions.isShown
  - [X] VerticalAxisContainerOptions.isShown
  - [X] VerticalAxisContainerOptions.isInputGridLinesShown
...
- [X] Go Changes start with:
  - [X] Rename horizontalAxisContainerOptions => inputAxisContainerOptions
  - [X] Rename verticalAxisContainerOptions => outputAxisContainerOptions
  - [X] Rename isInputGridLinesShown => isOutputAxisParallelGridLinesShown   
  - [X] Simplification and correcting around
    #+begin_src java
    ...
    #+end_src
    - [X] craft above according in each class to contain code suitable to situation
- [X] Test, validate, commit
- [X] Go over remaining todos
- [X] Test, validate, commit
...
- [X] add auto_layout examples as in coded_layout which skips labels, axis, grid
  - [X] Rename: ex35AnimalsBySeasonNoLabelsShown => ex35AnimalsBySeasonNoAxisNoGridLinesNoLabelsShown
  - [X] NO Add: ex36AnimalsBySeasonNoOutputAxisOrOutputAxisParallelGridLinesShown : should show all labels (input and output), show InputAxis and InputAxisParallelGridLines, NO OutputAxis NO OutputAxisParallelGridLines
  - [X] Add to test:
    - [X] ex35 converted to new layout, only column orientation, bar and line (2 images)
    - [X] NO ex36 new layout, column and row orientation, bar and line (4 images)
- [X] Save new images as appropriate
- [X] Test, validate, commit




** DONE ~20. Fix charts that are NOT working in new auto_layouter~

Failing auto-layouter has several categories

*** DONE Failing unexplained, but something to do with all-negative or all-positive values that do not start at 0

*bug fix in Interval resolved this error*

BOTH linechart is failing, ALL 4 Bar Chart OK
...
Core problem: At some point two intervals are intersected and the intersection (although non empty) fails because one end of the interval is Infinity

#+begin_example
The following StateError was thrown during paint():
Bad state: Intervals this=Interval(20.0, 40.0) and other=Interval(0.0, Infinity) do not intersect

The relevant error-causing widget was: 
  LineChart LineChart:file:///home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/lib/test/src/test_main.dart:1105:31
When the exception was thrown, this was the stack: 
#0      Interval.intersectionOrException (package:flutter_charts/src/util/util_dart.dart:242:7)
#1      Interval.positivePortionOrException (package:flutter_charts/src/util/util_dart.dart:247:46)
#2      Interval.portionForSignOfValue (package:flutter_charts/src/util/util_dart.dart:254:12)
#3      FromTransposing2DValueRange.subsetForSignOfPointOffsetBeforeAffmap (package:flutter_charts/src/util/util_flutter.dart:63:40)
#4      PointContainer.layoutUsingPointModelAffmapToPixels (package:flutter_charts/src/chart/cartesian/container/data_container.dart:662:79)
#5      LineAndPointContainer.layout (package:flutter_charts/src/chart/cartesian/chart_type/line/container/data_container.dart:190:36)
#6      BoxLayouter._layout_Descend (package:flutter_charts/src/morphic/container/container_layouter_base.dart:1110:13)
#7      BoxLayouter._layout_TopRecurse (package:flutter_charts/src/morphic/container/container_layouter_base.dart:1079:5)
#+end_example

*** DONE ex900 Failing due to all data 0.0 

Error assert failed in AffineRangedMap1D creation: The following assertion was thrown during paint():
'package:flutter_charts/src/util/util_dart.dart': Failed assertion: line 484 pos 17: 'fromRangeStart != fromRangeEnd': is not true.
...
*** DONE ex40 Failing due to ordinal user labels

*Core problem was: Bar chart must always extend Y values to 0.0, and ordinal user labels branch was not doing this*

failing in bar chart mode - overflow on the chart


#+begin_example
    'ex40LanguagesWithYOrdinalUserLabelsAndUserColors_barChart_column_nonStacked_newAutoLayouter',
#+end_example


*Compare to: ex75AnimalsBySeasonLegendIsRowStartTightItemIsRowStartTightItemChildrenPadded_barChart_column_nonStacked_newAutoLayouter*

The full log for ex75 is ONLY:

Launching lib/test/src/test_main.dart on Linux in debug mode...
Building Linux application...
✓ Built build/linux/x64/debug/bundle/flutter_charts
Gdk-Message: 19:36:33.807: Unable to load  from the cursor theme
Debug service listening on ws://127.0.0.1:37811/IiKqV4Rwdoo=/ws
Syncing files to device Linux...
 ### Log.Info: extractExampleDescriptorsFromDartDefine From lib/test/src/test_main.dart: Passed exampleDescriptors=[ex75_barChart_column_nonStacked_newAutoLayouter], length=1
 ### Log.Info: FlutterChartPainter. FLUTTER_CHART_PAINTER.PAINT: passed size=Size(1242.6, 560.0).
 ### Log.Info: First paint!
 ### Log.Info: get root: No cached _root found all the way to actual hierarchy-root.

*THE DIFFERENCE SEEMS TO BE EX40 DOES NOT EXTEND HORIZONTAL AXIS TO 0*
 

*Debug: When calling affmapBetweenRanges*


fromTransposing2DValueRange: ALWAYS SAME

- input int  (0.0, 100.0)
- output int (1.0, 10.0)
- orient Column

  .. *I THINK THE PROBLEM IS INPUT AND OUTPUT IS REVERSED ABOVE!!!*
  fromTransposing2DValueRange is created by calling:
  #+begin_src dart
...
to2DPixelRange (comes from constraints): BUT
ON ROOT hor=(0,1177.55), ver=(0, 496.0)

- horizontal = (0, 69.09) <== this is weird no???? we transform full axis interval to a subset (probably for bar)
- vertical   = (0, 496.0)


Examples of result pixelPointOffset when  horizontal =(0, 69.09), vertical =  (0, 496.0)

this=(12.5, 9.0) result=(34.5, 55.1)
this=(, 7) result=(34.5, 165.3)
this=(, 4) result=(34.5, 330.7)
this=(, 3) result=(34.5, 385.8 )
---
this=(37.5, 4) result=(34.5, 330.7) --- SAME AS (12.5, 4.0) ABOVE?? WHY??
this=(, 6) result=(, 220.4)
this=(, 9) result=(, 55.1)
this=(, 9) result=(, 55.1)
----
this=(62.5, 3) result=(34.5, 385.8)
this=(, 7) result=(, 165.3)
this=(, 6) result=(, 220.4)
this=(, 10) result=(, 0.0) !!
-----
this=(87.5, 9) result=(34.5, 55.1)
this=(, 6) result=(, 220.4)
this=(, 8) result=(, 110.2)
this=(, 1) result=(, 496.0) !!
----- END
...
This warning:
 ...
 ### Log.Info: extractExampleDescriptorsFromDartDefine From lib/test/src/test_main.dart: Passed exampleDescriptors=[ex40LanguagesWithYOrdinalUserLabelsAndUserColors_barChart_column_nonStacked_newAutoLayouter], length=1
### Log.Info: FlutterChartPainter. FLUTTER_CHART_PAINTER.PAINT: passed size=Size(1242.6, 560.0). <== same as ex75
### Log.Info: First paint!
### Log.Info: get root: No cached _root found all the way to actual hierarchy-root.
### Log.Warning: Layout size of this layouter Instance of 'Padder' is Size(71.1, 551.1), which does not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(71.1, 496.0)
### Log.Warning: Layout size of this layouter Instance of 'Row' is Size(284.4, 551.1), which does not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(284.4, 496.0)
### Log.Warning: Layout size of this layouter Instance of 'BarChartPointContainersBar' is Size(284.4, 551.1), which does not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(284.4, 496.0)


and

### Log.Warning: Layout size of this layouter Instance of 'WidthSizerLayouter' is Size(1177.6, 551.1), which does not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(1177.6, 496.0)
### Log.Warning: HeightSizerLayouter: layoutSize calculated is Size(1177.6, 551.1) exceeds, in height, the constraint length=496.0

and last

### Log.Warning: Layout size of this layouter Instance of 'BarChartRootContainer' is Size(1242.6, 615.1), which does not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(1242.6, 560.0)

then paint infos and warnings

### Log.Info: paintWarningIfLayoutOverflowsRootConstraints: rootOffset=Offset(0.0, 0.0), myPaintedRect=Rect.fromLTRB(0.0, 0.0, 1242.6, 615.1), rootConstraints=BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(1242.6, 560.0)


### Log.Info: paintWarningIfLayoutOverflowsRootConstraints: rootOffset=Offset(0.0, 0.0), myPaintedRect=Rect.fromLTRB(0.0, 20.0, 1242.6, 615.1), rootConstraints=BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(1242.6, 560.0)
### Log.Info: paintWarningIfLayoutOverflowsRootConstraints: rootOffset=Offset(0.0, 0.0), myPaintedRect=Rect.fromLTRB(65.0, 595.1, 1242.6, 612.1), rootConstraints=BoxContainerConstraints: minSize=Size(0.0, 0.0), maxSize=Size(1242.6, 560.0)
### Log.Warning: closestIntersectWith: This rectangle Rect.fromLTRB(0.0, 0.0, 1242.6, 560.0) does NOT overlap at all rectangle other = Rect.fromLTRB(1357.6, 510.0, 2535.1, 527.0).
### Log.Warning: closestIntersectWith: !(intersection.width >= widthIntersect) was INCORRECTLY true.intersection = Rect.fromLTRB(1357.6, 510.0, 1242.6, 527.0), widthIntersect=50.0

*** DONE ex72: INTENTIONALLY OVERFLOWING because Legend layout INTENTIONAL overflow makes chart ALSO overflow

*Reason: because (Legend, ChartBody) in ChartRootContainer are in TableLayout which uses Align.center by default.*

*Do not fix. this is intentionall presentation of how the layout overflow is handled*

The coded_layout oldManualLayout works, because it does it's own layout on top ChartRootContainer.
...
*** DONE Failing known reason - a chart without labels not implemented
...
*** DONE Failing to rotate or skip labels

all failing now work
...
** IN-PROGRESS ~YYY. RUNNING ISSUES and SHORT TODOs~
...
- [X] Implement ability to hide Container for layout and painting, or just painting
    



** DONE ~21. Bring dependencies up to latest~

- [X] Update Opensuse
- [X] Update flutter
- [X] Validate tests working, commit
- [X] Bring dependencies to latest
- [X] Validate tests working, commit
- 


** DONE Rename branch flutter_charts_v4_2023-01-26_convert_Presenters_to_BoxContainers to master_auto_layout

** IN-PROGRESS-NOW Implement LabelLayoutStrategy in auto_layout (rotate and skip labels when compressed) 

*** Investigation of how current (coded_layout) LabelLayoutStrategy works

**** The CL code uses for HorizontalAxisContainer:
...
**** The above has it's ~layout()~ code described here

- calls buildAndReplaceChildren which builds a list of labels named inputLabelContainerCLs
- iterates labels in inputLabelContainerCLs
  - for each label inputLabelContainer:
    - inputLabelContainer.applyParentConstraints(this, BoxContainerConstraints.infinity());
    - inputLabelContainer.layout();
    - If  !_isLabelOnIndexShown(xIndex) // xIndex of current inputLabelContainer
      - set skip on the label: inputLabelContainer.applyParentOrderedSkip(this, parentOrderedSkip);
      - 
    - inputLabelContainer.applyParentOffset // this moves labels to positions
- labelLayoutStrategy.reLayout(BoxContainerConstraints.unused());


**** The last step ~labelLayoutStrategy.reLayout~ is described in this section

DefaultIterativeLabelLayoutStrategy defines iteration of label size reduction as follows:
- depth=1: rotateLabels
- depth=2: skipLabels
- depth=3: decreaseLabelFont
- depth=4: decreaseLabelFont 2nd time

  
The DefaultIterativeLabelLayoutStrategy.reLayout code is recursive and does:

- Check if labels overlap, using ~if (!_adjustableLabelsContainer.labelsOverlap())~, return if not
- depending on depth, calls one of:
  - _reLayoutDecreaseLabelFont()
  - _reLayoutRotateLabels()
  - _reLayoutSkipLabels()


*lets only implement SkipLabels for now*

_reLayoutSkipLabels() sets ~_showEveryNthLabel~
sets ~labelLayoutStrategy.showEveryNthLabel~


***** Function of ~_adjustableLabelsContainer.labelsOverlap()~

- Goes over ONLY NOT SKIPPED labels (see ~applyParentOrderedSkip~)
- Checks the layout size for not overflowing

  
***** How is ~labelLayoutStrategy.showEveryNthLabel~ used? IN ~_isLabelOnIndexShown(int xIndex)~



*** TODO Copy LabelLayoutStrategy to LabelLayoutStrategyCL. Make CL code use the LabelLayoutStrategyCL, non-CL code use LabelLayoutStrategy
...
*** TODO Implement skip labels in auto_layout LabelLayoutStrategy

- use ex60LabelsIteration3 + auto_layout during implementation


**** Current classes that layout and paint horizontal axis and horizontal labels

***** AxisIntervalTicksAndLabelsDescriptor

Manages

- required this.chartOrientation,           (column, row)
- required ChartStacking chartStacking,     (stacked, nonStacked)
- required ChartViewModel chartViewModel,   (data values. can be used to construct data interval min/max and tick values)
- required this.dataDependency,             (dependend, independent = input, output)
- required bool extendAxisToOrigin,         (extend to 0)
- required Function valueToLabel,           (function from value => label. usually identity)
- required Function inverseTransform,       (if axis should show transformed values - e.g. log - this function is inverse - e.g. exp)
- List<String>? userLabels,                 (user labels if defined, override data)
...
The output/inputAxisDescriptor can be used to create the layouter for ticks and labels, the ~TransposingExternalTicks~:
...
- Inside the mixin _LabelsOrGridContainerBuilderMixin[implements _ChildrenProviderMixin]._buildTickedInputRangeRowOrOutputRangeColumn({
  #+begin_src java
  ...
  #+end_src
- *children are labels*, created by ~List<BoxContainer> _ChildrenProviderMixin._externallyTickedAxisLabelsOrGridLinesOnAxis~
- All of the below implement  ~List<BoxContainer> _ChildrenProviderMixin._externallyTickedAxisLabelsOrGridLinesOnAxis~:
  #+begin_example
  Implementations of _ChildrenProviderMixin  (all in  axislabels_axislines_gridlines_container.dart  - 9 usages found)
                        mixin _AxisLabelsProviderMixin implements _ChildrenProviderMixin, _LabelStyleMixin {
                        mixin _GridLinesProviderMixin implements _ChildrenProviderMixin {
            ...
            src.coded_layout.chart  (in axis_container.dart  - 2 usages found)
                        class HorizontalAxisContainerCL
                        class OutputAxisContainerCL
  #+end_example
  

*** TODO Implement rotate labels in  auto_layout LabelLayoutStrategy
*** TODO Ensure all tests work
*** TODO Commit and push

** TODO Add tag 2025-12-28_before_coded_layout_removal on master_auto_layout

- 
** TODO Remove all code related to coded_layout


** TODO ~25. HUGE: Containers to WARPS~

start doing Warp stuff:

- [ ] Start with everything is a transform!
  - [ ] 
- [ ] Rename Container to Warp
- [ ] Split flutter_charts to Warpic and flutter_charts
- [ ] 
- [ ] 

** TODO ~23. Beautify Charts: Add Spacing, make axis and grids thinner, what else????~

** TODO ~24. LARGE: Layout related changes~
...
** LATER ~Refactor / rename consideration~

 - [ ] *CONSIDER CAREFULLY:* in view_model.PointModel rename outputValue to trfdOutputDatum *but this is used a lot and related to a lot of other naming that would have to change as well*
 - [ ] *CONSIDER CAREFULLY:* rename around FromTransposing2DValueRange to FromTransposing2DDataInterval *but again this is used a lot and related to a lot*
 - [ ] Rename boundingRect to envelopeRect

** TODO ~27. Release version 0.2~

** LATER ~Add Chart Layout Templates; Follow Linter rules; Adjust Backups~

*** LATER Figure out how to create, name, and keep some basic chart layout templates, similar to LegendAndItemLayoutEnum - e.g. to pre-layout Y axis
...
- [ ] Exclude from backups:
  1. [ ] /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/integration_test/screenshots_tested # do it for flutter_charts*
  2. [ ] /home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts/test/tmp (may already be, see above)
  3. 

** TODO ~26. EVEN-LATER. : BoxContainer debuggability, introduce Singletons on places~
...
** NOTE Design of LayoutableBox, BoxLayouter, BoxContainer, BoxContainerHierarchy and possible changes

*NOTE: IN DART 'with MixinSomething' IMPLIES 'extends' (as if BY PASTING CODE) - this MIMICKs MULTIPLE INHERITANCE*

#+begin_src artist


    +---------------------------------------------------------------------------------------------+       +-----------------------------------------------------------------------------------------------------------------------------+
    | abstract class LayoutableBox                                                                |       |abstract class BoxContainerHierarchy extends Object with UniqueKeyedObjectsManager                                           |
    |   abstract: everything                                                                      |       |  abstract:                                                                                                                  |
    |   - ui.Size get layoutSize;                                                                 |       |    - void buildAndReplaceChildren();                                                                                        |
    |   - ui.Offset get offset;                                                                   |       |    - BoxContainer? _root;                                                                                                   |
    |   - void applyParentOffset(LayoutableBox caller, ui.Offset offset);                         |       |  concrete:                                                                                                                  |
    |   - void applyParentOrderedSkip(LayoutableBox caller, bool orderedSkip);                    |       |    - @override                                                                                                              |
    |   - void applyParentConstraints(LayoutableBox caller, BoxContainerConstraints constraints); |       |    - List<Keyed> get keyedMembers => _children;                                                                             |
    |   - void layout();                                                                          |       |    - final List<BoxContainer> __children = [];                                                                              |
    ...
    |                                                                                             |       |    - void buildAndReplaceChildrenDefault() {..empty}                                                                        |
    ...
    +---------------------------------------------------------------------------------------------+       +-----------------------------------------------------------------------------------------------------------------------------+
                                                     ^             ^                                                                                   ^
                                                     | implements  | implements LayoutableBox                                                          |
                                                     |             ---------------------------------------------------------------------------------   |
                                                     |                                                                                             |   |
                                                     |                                                                                             |   | extends
                                                     |        extends with BoxLayouter, also implements LayoutableBox                              |   |
                                                     |     +----------------------------------------------------------------------------------+    |   |
                                                     |     |                                                                                  |    |   |
                                                     |     V                                                                                  |    |   |
    +--------------------------------------------------------------------------------------+              +-----------------------------------+----+-----------------------------------------------+
    | mixin BoxLayouter implements LayoutableBox                                           |              | abstract class BoxContainer extends BoxContainerHierarchy with BoxLayouter             |
    |                                                                                      |              |   implements                                                                           |
    |                                                                                      |              |       LayoutableBox,                                                                   |
    |                                                                                      |              |       Keyed,                                                                           |
    ...
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |   todo-03 add abstract and concrete to both BoxLayouter and BoxContainer             |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |   Note:   pen is for inputting text. text is for figlets                             |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    |                                                                                      |              |                                                                                        |
    +--------------------------------------------------------------------------------------+              +----------------------------------------------------------------------------------------+
...
- abstract class ~LayoutableBox~ *top of hierarchy*
  - mixin ~BoxLayouter~ implements LayoutableBox                                       // *BoxLayouter  is LayoutableBox BUT RE-IMPLEMENTS*
  - abstract class ~BoxContainer~ extends ~BoxContainerHierarchy~ with ~LayoutableBox~ // *BoxContainer is LayoutableBox AND BoxContainerHierarchy*
  -                                                                                    // *BoxContainer is NOT BoxLayouter*
  -                                                                                    // *BoxLayouter  is NOT BoxContainer*


Why NOT:

class ~BoxLayouter~ extends ~LayoutableBox~ *AND* class ~BoxContainer~ extends ~LayoutableBox~???

- *that way BoxLayouter contains all ~LayoutableBox~ implementation AND ~BoxContainer~ also contains ~LayoutableBox~ implementation* ???
- *NOT POSSIBLE, THAT WOULD REQUIRE BoxContainer extends BOTH LayoutableBox and BoxContainerHierarchy*!!!! - *WOULD BE MULTIPLE INHERITANCE*.

  
*so we have to choose to either re-implement ~BoxContainerHierarchy in BoxContainer~ (alternative) OR re-implement ~LayoutableBox in BoxLayouter~ (current)*
*the latter (current) seems better, because LayoutableBox is a smaller implementation*

Example only:

#+BEGIN_SRC ditaa :file box-classes-uml.png :cmdline -E -r

     Parent Classes
                          +-------------------+                              +-----------------+
                          | BoxLayouter   c1AF|                              | cFFC   {s}      |
                          | mixin             |                              |                 |
                          +-------------------+                              |                 |
      O is bullet point-> | o field2          |                              |                 |
                          | o field3          |                     +------->|                 |
                          | o field4          |      dotted lines   :        |                 |
                          | o field5          |                     |        |                 |
                          +-------------------+                     |        +-----------------+
                          | o method1()       |      point marker   *
                          |                   |                     |        +-----------------+
                          +-------------------+                     |        : There           |
                           ^        ^       ^                       |        |                 |
                           |        |       |                       |        |                 |
     Child classes         |        |       |                       +--------|        +-------+|
           ...
           |              |         |                      |           |
    +------+--------+     | +-------+-------+      +-------+-------+   |
    | Child1    cF88|     | | Child2    cRED|      | Child3    c1FF|   |<------ We are gonna change
    +---------------+     | +---------------+      +---------------+   |        Child2 and Child3
    | field1        |     | | field2 Number |      | field1        |   |
    | field2        |     | | field1: Str   |      | field2        |   |
    | field3        |     | |       ^       |      +---------------+   |
    |               |     | |       |       |      | method        |   |
    +---------------+     | +---------------+      +---------------+   |
                          |         |                                  |
                          ...
                          +--------------------------------------------+
                                    |
                                    +----------------------------------------  Do not use colon, it
                                                                               is interpreted as dotted
                                                                               line

       Table created as "el table" can be placed to artist and looks good - maintains relative column width, nice
...
** 30. *IDEAS TO IMPLEMENT - table - is this done??*
