...
** IN-PROGRESS-NOW 5.2 Layouter improvements : New Layouters (Greedy, Center, Padding), Row: Packing, Align settings; overflow rectangle, and more
...
- [ ] ~Alignment~ new helper class that defines Alignment.topRight, Alignment.topStart, Alignment.topEnd, Alignment.bottomStart, Alignment.bottomEnd, Alignment.Center . layoutSize _top=-1, _bottom=1, _start=-1, _end=1.
  - [ ] It is an imaginary Square with sides 2x2, centered at 0, 0. Any point (offset) within the square represents the alignment. In a layout defined by ~Parent(child: Aligner(alignment: Alignment, widthFromChildBy: 1.0,  heightFromChildBy: 1.0, child: child))~, imagine Alignment is transformed (disproportionally) to ~Aligner~'s size ~alignerSize = Size(childWidth * widthFromChildBy, childHeight * heightFromChildBy, )~, let us call this ~AlignmentStretchToAligner~. It's ~alignX~, is between ~0 and AlignerSize.width~, and ~alignY~ between ~0 and alignerSize.height~. Then the child will be positioned at the Offset in aligner corresponding to ~Offset(alignX, alignY)~
  - [ ] Members alignX, alignY values between -1 and +1 for now. 
  - [ ] ~alignChildInSelf(Size childSize, Size selfSize)~ => *HOW DOES THIS CORRESPOND TO ALIGNERsIZE ABOVE????*
...
- [ ] ~Aligner~, a Layouter that aligns single child - no painting needed
  ...
  - [ ] Members all final
    - ~sizeFromChildBy.width~ (>=1),
    - ~sizeFromChildBy.height~ (>=1), self layout size should be bigger than child layout size by this much
    - STORE IT AS ~Size sizeFromChildBy~
    - ~alignment~
  - [ ] ~Aligner~ Constructor is one-child, plus 3 required defaulting args for "by" ~widthFromChildBy~  ~heightFromChildBy~ (no default) + ~Alignment~ (default center)
  ...
  - [ ] constraint push to child: if multiple set, create a constraint from self constraint, which is multiplied in directions by ~1/sizeFromChildBy.width, 1/sizeFromChildBy.height~, set on child, else copy self constraint. apply the constraint on child
  - [ ] If ~sizeFromChildBy.width~ (>=1), ~sizeFromChildBy.height~ ( is set
    - [ ] *child position function*: we have childSize. WE KNOW ~selfSize = childSize * sizeFromChildBy~ child position (offset) is inside selfSize: Rect childPositionInMe = ~alignChildInSelf(Size childSize, Size selfSize)~
    - [ ] *offset child function*: use default
    - [ ] *self layout size* : return  ~childLayoutSize * sizeFromChildBy~ (result=multiplication of sizes in both height and width direction) 
