...
** IN-PROGRESS-NOW 5.1 Refactor using of parent sandbox on BoxLayouter

*** Members which should always be set by parent, use name 'applyParent', and add explicit caller test

  First removed parent sandbox
  
- [ ] childrenInLayoutOrderGreedyLast is used purely in layout. Move as private in BoxLayouter
- [ ] same for addedSizeOfAllChildren
...
- [X] constraints
- [ ] layoutSize - this does NOT need apply - just late final layoutSize - should be set only once, and then get only.

Nutshell what we need:

Added checks for parent on applyParentOffset. Same thing should be on 
constraints (apply+getter), orderedSkip(apply + getter),  layoutSize (getter only, set once thru late final).
