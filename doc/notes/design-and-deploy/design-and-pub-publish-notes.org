...
** IN-PROGRESS-NOW 4. Define and implement the Layouter algorithm
...
#+BEGIN_SRC python
  def layout(Constraint sizeConstraintFromParent):     # 1. sizeConstraints go down (as this is called in parent context)
    if this is leaf:
...
#+end_src
** IN-PROGRESS 5. Container and Morph: Deeper refactorings to make flutter_charts a bit like Morphic (marginally like it)
...
*** IN-PROGRESS-NOW Modify Container - Add Layout and Painter base class
*Lessons for Container*

See container_base_new.dart

*** IN-PROGRESS-NOW Core layout algorithm
*** IN-PROGRESS-NOW ~LayoutExpansion: Refactor LayoutExpansion to SizeConstraint and clarify role of SizeConstraint~
...
**** NOTE Split layout to layout and layoutImpl?

- ~Layout~ is base class
  - does NOT store size or shape, only returns from layout and layoutImpl
  - method ~Volume layoutImpl(covariant Constraints)~ is abstract. Specializations implement this
  - method ~Volume layout(covariant Constraints)~ is implemented ONLY here NOT in specializations. Is final. Specializations call this. This calls layoutImpl and maybe configures some constraints caching and debugging eventually. Caching and debugging is the only reason to s 


**** IN-PROGRESS-NOW Rename LayoutExpansion to BoxSizeConstraint (SizeConstraint equivalent of BoxConstraints to distinguish from BoxConstraints)
