...
*** IN-PROGRESS-NOW KEEP : ~layout()~ code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~newCoreLayout~,  but positioning code is still old, manual, copied from old code!!!
...
**** IN-PROGRESS-NOW Migrate the following classes 
...
*** IN-PROGRESS-NOW Remove argument from ~BoxContainer buildAndAddChildrenLateDuringParentLayout(covariant DataContainerBuildState buildStateDependentOnSiblingsLayout)~ and replace the passed object with ALayoutMessageToB


- probably move xTickXs and yTickYs from ChartRootContainer on YContainer and XContainer
- access them from any place they were used
*** TODO-NEXT Can we move creation of ChartRootContainer from newCoreLayout to constructor???

This would also move YContainer build there. Only XContainer and DataContainer needs the build method it seems !!!

*** IN-PROGRESS-NOW Add AnchoredRootContainer which only contains ChartRootContainer. ChartRootContainer is ALWAYS REBUILD

*** IN-PROGRESS-NOW Remove manual layout baseclass

*** IN-PROGRESS-NOW Rename newCoreLayout to layout

*** IN-PROGRESS-NOW Start using BuildStateDependentOnSiblingsLayout everywhere instead of null.
