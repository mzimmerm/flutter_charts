...
*** TODO ChartData contains List<DataSeriesMixin> (think of it as Stack<DataSeriesMixin>)
*** TODO DataSeriesMixin corresponds to dataRow
*** TODO DataSeriesMixin contains List<DataPoint>
*** TODO DataPoint can be any object - sort of a naked user object
*** TODO DataSeriesMixin is a mixin which provides functions that allow to extract values from DataPoint
**** DataSeriesMixin.xValue(DataPoint, pointIndexInSeries) =>  whatever value at DataPoint is considered x // similar for Y
**** DataSeriesMixinMixin provides ~default functions which extract xLabel, yLabel, yTransform~
***** DataSeriesMixin.xLabel(DataPoint, pointIndexInSeries) => DataSeriesMixin.xValue(DataPoint, pointIndexInSeries) // similar for Y
*** TODO ~DataSeriesMixin must be extended as MyDataSeriesMixin, implementing at least 2 methods: xValue and yValue~
*** TODO *RENAME OWN_SCALE TO Y_VALUES_SCALE, TO_SCALE TO TO_DISPLAY_SCALE*
*** TODO *ADD 2 EXAMPLES WITH VARIOUS DATA SIMILAR TO RANDOM DATA*
*** TODO *DESIGN OF LOGARITHMIC SCALING WITH EXISTING LABEL FORMATTERS AND SCALERS*
- implemented by ovewriting DataSeriesMixin.yTransform
- only applies if yValues are numeric
- only applies on line chart?
- yValue is the original value
- values passed to existing label formatters and scalers is yTransform(yValue)
...
*** TODO Rename ChartData to ChartModel

*** TODO Rename ChartDataRows to ChartModelRows

*** TODO What options are controlled on the ChartModel level? What on the ChartModelRows level?
