...
- [X] Add PaddingLayouter that can be used around single child to pad with space - no painting needed
  ...
  - [X] Test work
- [ ] Add CenterLayouter that centers single child - no painting needed
  - [ ] Extends Positioning
  - [ ] Constructor is one-child, plus 2 optional arguments childWidthMultiple and childHeightMultiple.
  - [ ] If multiple along axis not set
    - [ ] pass self constraint to child, default
    - [ ] Child position is set in the middle of self constraint (may overflow of course)
  ...
  - [ ] Use CenterLayouter in Legend and LegendItemContainer to make sure they are usable.
  - [ ] Test work

** TODO-NEXT 6. REST Containers 1. : ~First migrate all existing Containers to use new Layout methods, but still use manual layout as is. Do NOT deal with Presenter, VerticalBarPresenter, LineAndHotspotPresenter~
