...
*** Separate integration test images between 'oldManualLayouter' and 'newAutoLayouter'

Before changes:

screenshots_expected> ls -1 *oldManual* | wc
     29      29    2574
screenshots_expected> ls -1 *newAuto* | wc
     18      18    1647

Images currently in *origAllTestedOld*

New enum *onlySupportedOld* for charts that do not work in new auto_layout
...
// ex30 : only add line chart to allSupportedNew (tests explicit LabelLayoutStrategy, but does not add anything new, as the LabelLayoutStrategy is default anyway)
// ex32 : some fail in new, put only selected to new
// ex33 : all fail in new, put only selected to new
// ex34 : tests custom font on labels, no need to test layout
...
- [ ] Rename screenshot_create_test to screenshots_create_test
- [ ] Note only: If dart file ends with *_test.dart* it must (should?) have main()
- 
*** Core of the parent section
