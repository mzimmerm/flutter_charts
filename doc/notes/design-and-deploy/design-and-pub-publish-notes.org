...
*** LATER Replace y label formatter and scaler with a more elegant, see pilot project - I added one there. See todo-00 there
...
5. [ ] ~BoxContainerConstraints~ are never divided - in any Layouter, including ~RowLayouter~
...
7. [ ] *Important: actual children of container : Ensure (manually for now), that each ~BoxContainer~*:
   - EITHER uses non-overriden default ~newCoreLayout~ - Then it MUST wrap it's actual children in auto-layouter (~RowLayouter or ColumnLayouter~).
     - This means that the container has only *ONE IMMEDIATE CHILD* (~RowLayouter or ColumnLayouter~); the IMMEDIATE CHILD ~RowLayouter or ColumnLayouter~ then has *ONE OR MANY* actual children.  
...
9. [ ] Allow 0, 1 children to be greedy in MainLayoutAxis, otherwise exception
...
** IN-PROGRESS Sequence of calls of old and new layout

- ~FlutterChartPainter.paint~ calls
- ~ChartRootContainer.paint~ calls
- ~ChartRootContainer.layout~ which creates in order
- The ~LegendContainer~, ~YContainer~, ~XContainer~ etc.
  - After each container is created, it's ~layout~ is immediately called.

IMPORTANT: ~BoxContainer~ extends ~BoxLayouter and BoxContainerHierarchy mixins~, ADDS ONLY METHODs ~paint(Canvas)~ and ~buildContainerOrSelf~ to ~BoxLayouter~

** IN-PROGRESS Questions to investigate
...
- [X] BoxLayouter vs LayoutableBox:
  - ~LayoutableBox~:
    - abstract class with empty implementations of all methods. It's interface is implemented(so overriden) by *both* ~BoxLayouter~ and ~BoxContainer~
    #+begin_src dart
      abstract class LayoutableBox {
...
** DONE 5. LEGEND LAYOUT: Use new Layouter algorithm + new Box/Column wrap ON LEGEND Containers (LegendContainer) instead of custom code. REST of Containers (ChartRootContainer, X/YContainer, DataContainer) use original code.

*** DONE Use the new Layout on Containers used in LEGEND = LegendContainer, LegendItemContainer, LabelContainer

**** DONE Use RowLayouter in LegendContainer, LegendItemContainer, LabelContainer

**** DONE Continue new Layout on LEGEND

- [X] *in build, try to nest like in Flutter*
  - [X] *FOR THAT, WE NEED TO*
    - [X] *ENFORCE ALL BoxContainer CONSTRUCTORS TO ACCEPT REQUIRED 'CHILDREN', but if not set, call buildContainerOrSelf in constructor, and attach as children*
 

*** DONE Copied snippets of original buildContainerOrSelf, which have value to see how original layout code was migrated, for migration of other Container classes like XContainer

#+name: LegendItemContainer
#+begin_src dart
...
#+end_src
** DONE 5.1 Members which should always be set by parent, use name 'applyParent', and add explicit caller test

- [X] First remove parent sandbox

Nutshell what we need:

*Add checks for parent on applyParentOffset. Same thing should be on constraints (apply+getter), orderedSkip(apply + getter),  layoutSize (getter only, set once thru late final).*

- [X] childrenInLayoutOrderGreedyLast is used purely in layout. Move as private in BoxLayouter
- [X] same for addedSizeOfAllChildren

On BoxLayouter:

- [X] all values that must be set by parent, should have
  - [X] 'applyParent' methods on BoxLayouter, which act as setters (e.g. applyParentOffset) and pass 2 values:
    - [X] (Object caller, value)
    - [X] these will generally check that parent === caller
  - [X] supporting elements (e.g. _offset) - this SHOULD be final on orderedSkip, constraints, layoutSize but NOT _offset which needs to keep changing. (but CANNOT be final, as we are setting it in 'apply')
  - [X] getter which returns the private (e.g. _offset)

What are those?
- [X] offset - does not even need a getter ??? . 
- [X] orderedSkip - rename orderedSkip
- [X] constraints
- [X] layoutSize - this does NOT need apply - just late final layoutSize - should be set only once, and then get only.


*** NO : Can we make layoutSize result of layout and not store on newCoreLayout?

- NO: layoutSize member: Make still available as late final on BoxLayouter, set it after return in case it is needed later. Always set just after return from newCoreLayout.
- NO: constraints member: Remove  
  

  

** IN-PROGRESS-NOW 5.2 Layouter improvements : Debugging, RowLayouter Packing, Lineup, and other settings

Debugging notes on why using a 'right alignment' on legend, the legend ends up outside the drawing area:

- In _layoutChildrenUsingOneDimAxisLayouter_As_LayedOutSegments, then in _notLeaf_OffsetChildrenAccordingToLayouter , RowLayouter _convertLayedOutSegmentsToOffsets creates offsets on the right -> layedOutOffsets =  300, 320 start, hugely offset to the right
...
- Then ~_notLeaf_SetSize_As_OuterBoundOf_OffsettedChildren~ sets layoutSize of the RowLayouter to a childrenOuterRectangle, so for example, ~layoutSize = childrenOuterRectangle.size~ => (74=20+54, 20) *which is what we want. What pushes it so much to the right?  The already set 300, 320 offset on the rectangle and label. THIS IS THE RESULT OF HAVING A FULL CONSTRAINT PASSED TO ALL CHILDREN IN RowLayouter. So I think the core solution is not do that, and divide the constraints*
- ~Solution already suggested below~ - We should have RowLayouterExpand = true/false, false default, if false, the layedOutOffsets, result of _convertLayedOutSegmentsToOffsets should be moved to start with it's first child. So layedOutOffsets would be set to 0, 20.
- The RowLayouter layoutSize should be size of rect (20) + size of label (64)
- also expand=false is NOT compatible with packing = flow


 I *think the above proves the approach below. We need to introduce the even distribution of constraints to children, in addition to have the expandToConstraintMax false as default.*
...
- [ ] also note and add to code : expandToConstraintMax=false is NOT compatible with packing = loose - add that note somewhere
- [ ] LATER :Add to OneDimLayoutProperties
  - [ ] doExpandToConstraintMax: true and false, default true *I think this is for later or never*
    - [ ] If true, use all available constraint (former totalSize) - this adds padding - this is how it works now!
    - [ ] If false, do not add any padding
  - [ ] isConstraintDividedToChildren: default false
    - [ ] If true, divide and pass constraints to children using some ratio.
    - [ ] If false, current behavior. We still need to pass constraints to children in newCoreLayout, that is missing.
- [X] Add ability to set packing, snap, expandToConstraintMax, constraintDivisionToChildren  on RowLayouter and ColumnLayouter.
- [ ] LATER BoxContainer debuggability
  - [ ] Add member "nodeId" on constructor (nullable), should be member in Hierarchy, and add to constructors at least those used in Legend
...
- [ ] LATER Add to  LineSegment a field 'isAddedPadding' and 'isSegment'. That way we can clearly see what is what
...
- [X] Add check to newCoreLayout, root section, if RowLayouter OR ColumnLaoter is deeper, and Lineup set to anything but 'start' and Packing 'snap', replace the Row/ColumnLayouter with a new copy with 'start' and 'snap'.
  - [X] Test it on some combinations in legend layout
    - [X] nested Lineup.end on Row
    - [X] we also have Column example to test
- [X] One remaining todo-00-last-last
  - [X] Find problem or comment
  - [X] Make sure Tests work
  - [X] Commit and push
- [X] Add ability in LegendContainer to create example with specific layouts of ~LegendContainer~
  - [X] We need the ability to define different LegendContainer and LegendItemContainer example variations, and a way to push from this code which example we want, and in the code, implement the example variations.
  ...
  - [X] Add 3 examples (only use LineChart) which overrides creation of ~LegendContainer~
    - [X] LegendContainer starts with RowLayouter    'start',  'snap',  LegendItemContainer is Row 'start', 'snap' (default desired and tested)
    - [X] LegendContainer starts with RowLayouter    'center', 'loose', LegendItemContainer is Row 'end', 'loose' (overriden to 'start', 'snap' , so noop)
    - [X] LegendContainer starts with ColumnLayouter 'start',  'snap',  LegendItemContainer is Row 'start', 'snap' (is top, so is NOT overriden, so must be as intended!)
    - [X] LegendContainer starts with ColumnLayouter 'start',  'loose', LegendItemContainer is Row 'start', 'snap' (as above)
    - [X] Create new test images, check them
    - [X] Add the above to representative tests in ~ExamplesDescriptor~ (line charts only)
    - [X] Run all tests
    - [X] commit and push
- [-] *Add proper layouter and container hierarchy for offsetting and non-offsetting layouters. In the end, there should be nothing related to ~main/crossAxisLayoutProperties~ in BoxLaouter or BoxContainer*
  - [-] ~BoxContainer~ will serve as a default ~NonOffsettingBoxLayouter~
    - [ ] Find the method ~YYY~ which does layout of children and performs their offset - must be using ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~
      - [ ] There are *multiple methods* that address ~main/crossAxisLayoutProperties OR RowLayouter OR ColumnLayouter~:
        1. ~_ifRoot_Force_Deeper_Row_And_Column_LayoutProperties_To_Non_Offsetting~ called in ~newCoreLayout isRoot~ condition - MOVE THIS
      ...
      - [ ] Call hierarchy of the methods YYY
        1. ~_ifRoot_Force_Deeper_Row_And_Column_LayoutProperties_To_Non_Offsetting~ IN
           - ~newCoreLayout~ 
  ...
  - [-] Make ~Row and ColumnLayouter~ extend ~RollingOffsettingBoxLayouter~
    - [X] Move ~YYY~ from BoxLayouter to ~RollingOffsettingBoxLayouter~
    - [ ] Move ~mainAxisLayoutProperties and many friends (which? list here first)~ from BoxLayouter to ~RollingOffsetBoxLayouter~
      - [ ] TODO friend 1 ??? 
      - [ ] friend 2
  - [ ] REVIEW CHANGES AND TODOS - RESOVE ABSTRACT OR NO-OP METHODS WHICH 'KNOW' ABOUT ROW_LAYOUTER OR OTHER THINGS known to extensions like RollingOffsettingBoxContainer.
  - [ ] *In the end, there should be nothing related to ~main/crossAxisLayoutProperties, Row, Column~ in BoxLayouter or BoxContainer*
  - [ ] Make sure Tests work
  - [ ] commit and push
- [ ] Rename Lineup to Align
- [ ] Rename Packing.snap to Packing.tight
- [ ] Add a mid-class between BoxContainer and RowLayouter/ColumnLayouter, and move things from BoxLayouter 
- [ ] Make sure Tests work
- [ ] Add container Greedy, equivalent of Expanded with ratio
  - [ ] Change layout algorithm to process non-expanded first, GIVE EXPANDED RATIO OF remaining CONSTRAINT - THIS REQUIRES A NEW STEP
  - [ ] Test on LegendContainer, by wrapping one or two short text LegendItemContainers into Greedy, they should eat up space despite being small.
- [ ] Make sure Tests work
- [ ] Support to show yellow and black rectangle on overflow:
  - [ ] Add an object on BoxContainer, with a Rectangle and a flag that would be set on overflows of sizes over offsets. 
  - [ ] Add some code, somehow,  Whereever we now check for layoutSize overflow constraints, set the object.
  - [ ] In top paint method, check for the flag and if true, paint a yellow/black rectangle there.
  - [ ] Add top paint method as super to all paint methods.
  - [ ] Test this on some forced layouts - *need to temporarlily revert the fix for nested layouts OR make one legend long!!*
- [ ] Make sure Tests work
- [ ] Add PaddingLayouter that can be used around boxes to pad with space - no painting needed?
- [ ] Use PaddingLayouter in Legend and LegendItemContainer to make sure they can be nested.
- [ ] LATER: Add Layout which wraps! (call it WrapLayouter). This is important for long legends.


Older plan, still valid:

- [X] Resolve why RowLayouter Packing.loose behaves same as Packing.snap !! Reason: Simplicity of single-pass layout. Cannot have Row in Row with nested Row being 'end aligned'.
- [ ] MILESTONE !!!!!! vvvvvvv 
- [ ] Read https://blog.logrocket.com/flutter-layouts-guide-margins-padding/
- [X] Create proper RowLayouter, add comments
  - [X] Basically, need to make sure I can pass constructor arguments to set pack/loose/align - see what they are
- [X] Make sure default row layouter on container is Row, which aligns left and top (basicall, sets parent offset = 0!!
- [X] Create proper ColumnLayouter, add comments
- [ ] Apply RowLayouter on chart LEGEND:
  - [ ] using a 'flow' type row layout so it fills the whole available width
  - [ ] ALSO add 4px space around rectangle using Padding 
  - [ ] also add 4px padding around the whole LEGEND, so it is separated from the chart
- [ ] This will test the non-greedy portion of layout some more

- [ ] --- MILESTONE
- [ ] make ChartOptions a Singleton!!! OR add ChartOptions as parameter on BoxContainer. ****Singleton is much prefered*
- [ ] !!!! Start converting all containers to BoxContainers and newCoreLayout, BUT KEEP MANUAL LAYOUT ON ALL CONTAINERS EXCEPT LEGEND
- [ ] Address some relevant todo-01-last
- [ ] Address and test greedy children layout by creating some examples of variously positioned Legend : Top, Right. and show landscape mode.

- [ ] when done:
  - [ ] start doing Morph stuff: start with everything is a transform!
 

** IN-PROGRESS-NOW ERROR investigate

MServiceFlutterDriver: Connected to Flutter application.
W/Parcel  ( 7311): Expecting binder but got null!
I/flutter ( 7311): ══╡ EXCEPTION CAUGHT BY RENDERING LIBRARY
╞═════════════════════════════════════════════════════════
I/flutter ( 7311): The following StateError was thrown during paint():
I/flutter ( 7311): Bad state: Layout size of this layouter Instance of 'RowLayouter' is Size(866.9,
20.0), which does
I/flutter ( 7311): not fit inside it's constraints BoxContainerConstraints: minSize=Size(0.0, 0.0),
maxSize=Size(374.4,
I/flutter ( 7311): 499.4)
I/flutter ( 7311): 
I/flutter ( 7311): The relevant error-causing widget was:
I/flutter ( 7311):   VerticalBarChart
I/flutter ( 7311):
VerticalBarChart:file:///home/mzimmermann/dev/my-projects-source/public-on-github/flutter_charts_v2/exa
mple/lib/main.dart:915:45
I/flutter ( 7311): 
I/flutter ( 7311): When the exception was thrown, this was the stack:
I/flutter ( 7311): #0      BoxLayouter._check_IfMySizeFit_WithinConstraints
(package:flutter_charts/src/chart/container_layouter_base.dart:747:7)
I/flutter ( 7311): #1
BoxLayouter._postDescend_IfLeaf_SetMySize_NotLeaf_OffsetImmediateChildrenInMe_ThenSetMySize_Finally_Che
ckIfMySizeWithinConstraints (package:flutter_charts/src/chart/container_layouter_base.dart:672:5)
I/flutter ( 7311): #2      BoxLayouter.newCoreLayout
(package:flutter_charts/src/chart/container_layouter_base.dart:582:5)
I/flutter ( 7311): #3      BoxLayouter.newCoreLayout
(package:flutter_charts/src/chart/container_layouter_base.dart:578:13)
I/flutter ( 7311): #4      LegendContainer.layout
(package:flutter_charts/src/chart/container.dart:1371:5)
I/flutter ( 7311): #5      ChartRootContainer.layout
(package:flutter_charts/src/chart/container.dart:178:21)
I/flutter ( 7311): #6      ChartRootContainer.paint
(package:flutter_charts/src/chart/container.dart:289:5)
I/flutter ( 7311): #7      FlutterChartPainter.paint
(package:flutter_charts/src/chart/painter.dart:68:24)
I/flutter ( 7311): #8      RenderCustomPaint._paintWithPainter
(package:flutter/src/rendering/custom_paint.dart:571:13)
I/flutter ( 7311): #9      RenderCustomPaint.paint
(package:flutter/src/rendering/custom_paint.dart:613:7)
I/flutter ( 7311): #10     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #11     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #12     RenderBoxContainerDefaultsMixin.defaultPaint
(package:flutter/src/rendering/box.dart:2900:15)
I/flutter ( 7311): #13     RenderFlex.paint (package:flutter/src/rendering/flex.dart:1088:7)
I/flutter ( 7311): #14     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #15     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #16     RenderBoxContainerDefaultsMixin.defaultPaint
(package:flutter/src/rendering/box.dart:2900:15)
I/flutter ( 7311): #17     RenderFlex.paint (package:flutter/src/rendering/flex.dart:1088:7)
I/flutter ( 7311): #18     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #19     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #20     RenderShiftedBox.paint
(package:flutter/src/rendering/shifted_box.dart:84:15)
I/flutter ( 7311): #21     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #22     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #23     RenderBoxContainerDefaultsMixin.defaultPaint
(package:flutter/src/rendering/box.dart:2900:15)
I/flutter ( 7311): #24     RenderCustomMultiChildLayoutBox.paint
(package:flutter/src/rendering/custom_layout.dart:415:5)
I/flutter ( 7311): #25     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #26     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #27     RenderProxyBoxMixin.paint
(package:flutter/src/rendering/proxy_box.dart:144:15)
I/flutter ( 7311): #28     _RenderInkFeatures.paint (package:flutter/src/material/material.dart:609:11)
I/flutter ( 7311): #29     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #30     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #31     RenderProxyBoxMixin.paint
(package:flutter/src/rendering/proxy_box.dart:144:15)
I/flutter ( 7311): #32     RenderPhysicalModel.paint.<anonymous closure>
(package:flutter/src/rendering/proxy_box.dart:2098:15)
I/flutter ( 7311): #33     PaintingContext.pushClipRRect
(package:flutter/src/rendering/object.dart:550:14)
I/flutter ( 7311): #34     RenderPhysicalModel.paint
(package:flutter/src/rendering/proxy_box.dart:2085:21)
I/flutter ( 7311): #35     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #36     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #37     RenderProxyBoxMixin.paint
(package:flutter/src/rendering/proxy_box.dart:144:15)
I/flutter ( 7311): #38     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #39     PaintingContext.paintChild
(package:flutter/src/rendering/object.dart:239:13)
I/flutter ( 7311): #40     RenderProxyBoxMixin.paint
(package:flutter/src/rendering/proxy_box.dart:144:15)
I/flutter ( 7311): #41     RenderObject._paintWithContext
(package:flutter/src/rendering/object.dart:2796:7)
I/flutter ( 7311): #42     PaintingContext._repaintCompositedChild
(package:flutter/src/rendering/object.dart:155:11)
I/flutter ( 7311): #43     PaintingContext.repaintCompositedChild
(package:flutter/src/rendering/object.dart:98:5)
I/flutter ( 7311): #44     PaintingContext._compositeChild
(package:flutter/src/rendering/object.dart:250:7)
I/flutter ( 7311): #45     PaintingContext.paintChild (package:flutter/src/rendering/object.dart:231:7)
I/
...
*** TODO How to do this code migration to new layout performed manually
...
the manually divided and allocated space for the child
   ...
   - 2.4 call NOOO ~addChildToHierarchyDeprecated(child)~ *MAYBE WE WILL HAVE TO UNDEPRECATE, OR JUST USE*
