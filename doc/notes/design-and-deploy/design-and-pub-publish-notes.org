...
- When passed to constructor, then they are set to non-null in argument. 
- When not passed, they are not used at all
  - yScaler.labelInfos is used instead
    ...
    - yScaler notes  
      ...
      - *so the labels are simple distribution of as many labels as needed between data min/max - ChartData.yLabels are NEVER USED when not set in constructor.*
        ...
        - add to validate, interplay between useUserProvidedYLabels and yLables set / not set
        - rename value in params in range to pointsColumnsPointsValues
        - maybe rename useUserProvidedYLabels to useYLabelsFromChartData *maybe remove useUserProvidedYLabels - if yLabels are set in constructor use them, otherwise create labels from data*
