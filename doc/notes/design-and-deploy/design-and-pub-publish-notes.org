...
13. [ ] Run all tests.
14. [ ] Testing framework speedup: Change the testing framework to only BUILD ONCE then run multiple tests.
15. [ ] Major but EASY REFACTORING
    1. [ ] Renames:
       1. [ ] CHART_TYPE_TO_SHOW -> CHART_TYPE (everywhere, even in IntelliJ files)
       2. [ ] chartTypeToShow -> chartType
       3. [ ] IS_USE_OLD_DATA_CONTAINER -> IS_OLD_LAYOUTER  (everywhere, even in IntelliJ files)
       4. [ ] YContainer -> OutputAxisContainer
       5. [ ] XContainer -> InputAxisContainer
       6. [ ] remove all 'new' from .dart file names
       7. [ ] remove most 'as name' from .dart files imports
       8. [ ] replace usage of enumName(exampleComboToRun.item1) with exampleComboToRun.item1.name - enumName is no longer needed
16. [ ] Code changes start using Transposing instead of Regular ALMOST EVERYWHERE - FIRST ASK : WHERE NOT?
    1. [ ] WHERE YES?
    2. [ ] WHERE NOT?
    3. [ ] MAKE CODE CHANGES
17. [ ] Change  Row -> Transposing.Row and  ExternalTicksRow -> TransposingExternalTicks.Row in code, but NOT TransposingSizer
18. [ ] More steps here, or should all work for both column and row???
19. [ ] Add tests for PointOffset lextr method
20. [ ] Replace PointOffset lextr method and use with proper 2d transforms thought through in Terra Cotta
21. [ ] 
