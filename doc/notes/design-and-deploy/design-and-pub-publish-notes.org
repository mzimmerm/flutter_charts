...
Study code around current ParentOrderedSkip - there is some, even in new layout. What does it do? It uses various variable names in Options containing 'isShown', which are then used in 'layout', 'applyParentOffset' and 'paint'. If isShown is false, those methods are no-op - they return before they can do anything.
...
Analyzes of current code (this includes the new layout BUT active use of 'isShown' in new layout is ONLY in ~LegendContainer~ *constructor*, which calls ~applyParentOrderedSkip~
  ...
  - [ ] Options set 'isShown' with various naming changes on:
    - [ ] LegendOptions.isLegendContainerShown
    - [ ] HorizontalAxisContainerOptions.isShown
    - [ ] VerticalAxisContainerOptions.isShown
    - [ ] VerticalAxisContainerOptions.isInputGridLinesShown
  - [ ] The options are used in coded_layout examples to convert to no-op all of:
    - [ ] ~layout~
    - [ ] ~applyParentOffset~
    - [ ] ~paint~
  - [ ] There is ONE *auto_layout* container that sets parentOrderedSkip:
    - [ ] ~LegendContainer constructor~, there is this code:
      ...
      #+end_src
  - [ ] ~BoxLayouter._orderedSkip~ is a field which todo-now



  
    
- [ ] add class ParentOrderedSkip beside BoxLayouter mixin
  - [ ] bool isSkipLayout
  - [ ] bool isSkipPaint
  - [ ] private generative constructor
  - [ ] const static skipPaint
  - [ ] const static skipLayoutAndPaint
  - [ ] const static skipNone (DEFAULT)
  - [ ] 
...
- [ ] review options related to skipping
  ...
  - [ ] add example as in coded_layout which skips labels, axis, grid
