...
10. [-] Start using Transposing in XContainer, YContainer
    ...
    - [ ] Test all TODO MORNING.
    - [X] Next For Column, Row, ExternalTicksColumn and Row
11. [ ] column orientation: Run all tests in column orientation: Status: XContainer and YContainer use Transposing code IN FULL, BUT ONLY TESTED IN COLUMN MODE.
12. [ ] row orientation: Check it out. How does it look??
    - [ ] Probably some issues with transposed X, Y axis? Analyse and debug.
      - [ ] 
13. [ ] Code changes start using Transposing instead of Regular ALMOST EVERYWHERE - FIRST ASK : WHERE NOT?
    1. [ ] WHERE YES?
    2. [ ] WHERE NOT?
    3. [ ] MAKE CODE CHANGES
14. [ ] Change  Row -> Transposing.Row and  ExternalTicksRow -> TransposingExternalTicks.Row in code, but NOT TransposingSizer 
15. [ ] More steps here, or should all work for both column and row???
16. [ ] Add tests for PointOffset lextr method
17. [ ] Replace PointOffset lextr method and use with proper 2d transforms thought through in Terra Cotta
