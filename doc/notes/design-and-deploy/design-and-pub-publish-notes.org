...
** IN-PROGRESS-NOW 8. REST Containers 2. : ~Presenter - VerticalBarPresenter, LineAndHotspotPresenter - convert to BoxContainers~
...
*** IN-PROGRESS-NOW Create a *completely new* BoxContainers for everything. For now, keep ~PointPresenter~ s and related.
...
- [ ] ~TableLayouterCells~
  - [ ] Manages constraints for TableLayouter during layout
  - [ ] ~constructor-todo~
  - [ ] NOOO - we create ~TableLayouter.children~ in that order Member ~List<List<TableLayouterCell>> tableLayouterCellList~
  - [ ] Method ~find_cell_on(row, column) => TableLayouterCell on row, column~
  - [ ] Method ~calculate_used_width_on_column(row, column)~
    - [ ] go over all irows in (irow, column) except irow=row, where cell.isAlreadyLayedOut, result = layoutSize.width max
  - [ ] Method ~calculate_used_height_on_row(row, column)~
...
- [ ] ~TableLayouter~ extends same class as RollingPositioning ?
  - [ ] constructor
    - [ ] passed ~required List<List<BoxContainers>> childrenTable~ representing rows and columns
    - [ ] passed ~required TableLayouterCells tableLayouterCells~ which must be the same size, and have  ~layoutSequence~, ~row~, ~column~ set correctly and uniquely
  - [ ] build ??? What is done here? ~add all members in  childrenTable~ ???
  - [ ] Member ~TableLayouterCells tableLayouterCells~
  - [ ] Member ~List<ContainerBox> childrenInLayoutSequence~ ????????
  - [ ] Member ~childrenTable~
  - [ ] ~layout~ (which method?)
    - [ ] does not set constraints on all children
    - [ ] iterates children NOT in children layoutSequence, but in ~layoutSequence~ HOW??
      - [ ] child.layout
      - [ ] after each child layout, update self, then calculate and set the next cell's constraints:
        - [ ] ~child.isAlreadyLayedOut = true;~
        - [ ] ~nextCell = child.nextCellInLayoutSequence~
        - [ ] ~nextCell.constraints = calculate_cell_available_constraint_on_cell(nextCell.row, nextCell.column)~
- [ ] ~NewChartRootContainer with constructor that accepts TableLayouter and LegendContainer~ 
  - [ ] in build, NOT constructor, ~addChildren([Column(children: legendContainer, tableLayouter,])~
  - [ ] ~member TableLayouter~
  - [ ] ~member LegendContainer~
- [ ] in code
  - [ ] ~where? in makeChartRootContainer?~
  - [ ] 
  - [ ] ~legendContainer = create it~
  - [ ] ~create XContainer, YContainer, DataContainer, EmptyContainer~
  - [ ] ~create tableLayouterCellsYDEX, a TableLayouterCells object with 4 cells, in 2x2 arrangement as follows~
    - [ ] placing Y on 0,0 Data on 0,1  Empty on 1,0 and X on 1,1
    - [ ] configure row, column, layoutSequence, nextCellInLayoutSequence on each cell
  - [ ] ~create NewChartRootContainer, pass~
    - [ ] ~legendContainer: legendContainer~
    - [ ] ~tableLayouter: TableLayouter(childrenTable: [[yContainer, dataContainer], [emptyContainer], [xContainer]], tableLayouterCells: tableLayouterCellsYDEX,)~
      - the constructor should add them as column, also will create the rest of the hierarchy..
  - [ ] this should be IT!

    
Notes or Later:

- [ ] Keep model-like objects: ~PointsColumn~,  ~PointsColumns~,  ~StackableValuePoint~,  ~~,  ~~,  ~~, 
- [ ] Keep presented objects:  ~PointPresenter~,  ~PointPresenterCreator~,  ~PointPresentersColumn~,  ~PointPresentersColumns~, ~VerticalBarPointPresenter~,  ~VerticalBarLeafPointPresenterCreator~
- [ ] Create new objects from ~DataContainer~ down in a new library ~new_data_container.dart~, classes from bottom:
- [ ] ~NewValueContainer~, ~NewValuesColumnContainer~, ~NewValuesColumnsContainer~ - the last is equivalent to ~DataContainer~
- [ ] change anything using ~DataContainer~ to use ~ValuesColumnsContainer~ - what is it?
- [ ]
- [ ] In code and comments, review use of 'position' - it should mean 'extrapolated to pixels'
- [ ] In code and comments, selectively replace 'transform' to 'extrapolat'. 
- [ ] In code and comments, review use of 'scal' and replace with 'extrapolat' - there should be rare use of 'scal', only when both domain and range start with 0 and parameters such as 'scaleBy'
- [ ] Renames : DataRange etc => DataScope (means it can be dependend or independend)
- [ ] In the lextr methods, if the result is pixels, we can call it Codomain
  - [ ] ChartRootLayouter
 

-------------
Later: Rename: constraints.size. to constraints.


*** HOLD Notes on ~Presenters~ to ~BoxContainers~ conversion
...
** LATER 9. ~Add auto layouts that fully support building the chart, and build the chart fully from auto-layouted BoxContainers~

  
** TODO 10. ~Containers to MORPHS~
