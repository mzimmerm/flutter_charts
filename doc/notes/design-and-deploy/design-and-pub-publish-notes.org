...
- [ ] LATER :Add to LenghtsPositionerProperties
...
- [X] Make sure Tests work
- [ ] Try to remove the default newCoreLayout in Rolling for Greedy, and always use the version with assuming existence of _greedy. If results the same, use it even if slower.
- [ ] Support to show yellow and black rectangle on overflow:
  ...
  - [ ] v2
    ...
    - [ ] Add method ~paintIfLayoutProtrusion~ (to be used in any paint method) which checks if offset is beyond the  ~get topContainerConstraints~, 
      - [ ] if offset is out,
        - [ ] also find how far the offset is out, and create a rectangle ~protrudingInThisDirection~ inside  topContainerConstraints, on the general side of where offset is protruding
        - [ ] paint the  ~protrudingInThisDirection~ rectable
      - [ ] Add  ~paintIfLayoutProtrusion~ to every paint, or better, to paint abstract, and make sure all paint implementations call super. Maybe add to the place where we check if parentWantToHide...
    - [ ] 
    - [ ] 
  - 
