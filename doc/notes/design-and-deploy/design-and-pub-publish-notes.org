...
** IN-PROGRESS ~Speedup~ - around mappers, matrixes, and all methods where they are called
...
** IN-PROGRESS-NOW 11.2. ~Line chart~
...
** TODO ~Create a test app for super fast testing~
...
** LATER ~add dotted grid lines in it's own StackLayouter, then place axis lines into it's own container~

** LATER ~figure out how to create and keep some basic layout templates - e.g. to pre-layout Y axis~
