...
4. [ ] Run all tests in column orientation - because now the new hierarchy is already used
5. [ ] Add TransposingSizerLayouter.Width, Height between FromConstraintsSizerLayouter and HeightSizerLayouter
6. [ ] Run all tests in column orientation - because now the new hierarchy is already used
7. [ ] Start using Transposing in XContainer, YContainer
8. [ ] Make sure it works in column orientation - Run all tests
9. [ ] Pull out Padder from YContainer and DataContainer, and add it as in-place wrappers in ChartRootContainer to dataContainer and vertAxisContainer
10. [ ] NOW THE TRANSPOSE SHOULD WORK IN FULL?
11. [ ] Run all tests in column orientation
12. [ ] How is row orientation??
13. [ ] Start using Transposing instead of Regular ALMOST EVERYWHERE - FIRST ASK : WHERE NOT?
