...
- [-] ~Aligner~, a Layouter that aligns single child - no painting needed
  ...
  - [ ] *TODO TODO TODO* constraint push to child: if multiple set, create a constraint from self constraint, which is multiplied in directions by ~1/childSizeBy.width, 1/childSizeBy.height~, set on child, 
  ...
  - [ ] If multiple along axis NOT SET-  DO NOT ALLOW FOR NOW
    - [ ] constraint push to child: if multiple set, copy self constraint to child (this should be implemented in default). 
