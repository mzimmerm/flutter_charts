...
4. [ ] USE 2 TEST APPS : ONE FOR auto_layout, OTHER for coded_layout with the eventual goal of coded_layout gone.
   1) [ ] Copy test_main.dart to test_coded_layou_main.dart
   2) [ ] Copy screenshot_create_test.dart to coded_layout_screenshot_create_test.dart
      1. [ ] in the coded_layout version coded_layout_screenshot_create_test.dart, instead of calling  test_main.dart, call coded_layout_test_main.dart
   3) [ ] Copy 
   4) [ ] Modify run-all-tests.sh and related scripts to accept auto_layout and coded_layout options separately
      1. [ ]  run-all-tests.sh --auto-layout="ex1 ex2 group1 group2" --coded-layout"ex1 ex3 group3"
         ...
         + [ ] 
   5) [ ] TEST FOR SAMENESS AND COMMIT
      ...
      + [ ] COMMIT
   6) [ ] NEXT STEP IS INTRODUCE A COPY WITH MINOR CHANGE OF THE TEST FILES FOR CODED_LAYOUT AND AUTO_LAYOUT, SO THAT WE ONLY HAVE TO MAINTAIN ONE VERSION:
