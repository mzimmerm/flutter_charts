...
the manually divided and allocated space for the child
   ...
   - 2.4 call ~addChildToHierarchyDeprecated(child)~
...
1. Move one-by-one loop in ~2.1 and 2.4~ into  ~buildContainerOrSelf(BoxContainer parentBoxContainer)~
   - Notes:
     ...
     - In of ~2.4~ (addChildToHierarchyDeprecated), we have a new argument 'this', so we call ~addChildToHierarchyDeprecated(this, child)~
