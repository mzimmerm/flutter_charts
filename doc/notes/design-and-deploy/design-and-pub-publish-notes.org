...
1. [ ] Review where should the long build method (name?) be called after removing ~buildContainerOrSelf~
...
1. [ ] Review BoxContainer constructor use of ~children~
2. [ ] List classes that ~do not use children~ directly, but use member containers such as xContainer.
   1. [ ] 
   2. [ ] 
3. [ ] On the above classes, override children by adding concrete xContainer, yContainer etc
   1. [ ] 
4. [ ] Remove this.children from those classes and everywhere.

next:

1. [ ] BoxContainerHierarchy
   1. [ ] split children to get and set ????
   2. [ ]
