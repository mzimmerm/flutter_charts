...
*** IN-PROGRESS-NOW Example72 is different from previous version. Investigate and fix
...
*** IN-PROGRESS-NOW KEEP : ~layoutOLD()~ code and code migration: split ~layoutOLD~ to ~buildContainerOrSelf~, and ~post_NotLeaf_PositionChildren~,  but positioning code is still old, manual, copied from old code!!!
