...
- [ ] Put stuff back so tests work without changes
- [ ] Add check to newCoreLayout, if RowLayouter is deeper, and Lineup set to anything but 'start' and Packing 'snap', replace the RowLayouter with a new copy with 'start' and 'snap'. Probably at the beginning of newCoreLayout on root?
- [ ] Remove asserts around free space - there is TODO.
- [ ] Tests work
