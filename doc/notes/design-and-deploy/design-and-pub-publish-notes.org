...
** IN-PROGRESS-NOW ~15. Add dotted grid lines in it's own StackLayouter, then place axis lines into it's own container~ 4 days. Should start May28, end June 1.
...
- [ ] Add ~TransposingInputValuesGrid~ (perpendicular to TransposingInputAxis) and ~TransposingOutputValuesGrid~ first empty, then add lines in the same positions asmiddle of labels . *study how to externally position the lines - use the same layouter*
- [ ] *study how to externally position the lines in middle of labels - use the same layouter: use the highlighted: On InputAxis: *
  #+begin_src dart
          ...
          TransposingRoller.Column(
            ...
            children: [
              TransposingExternalTicks.Row(
                chartOrientation: chartViewModel.chartOrientation,
                // HERE , USE THE SAME TICKS FROM _INPUT_RANGE_DESCRIPTOR ALSO FOR THE GRID LINES IN HORIZONTAL DIRECTION
  ...
  #+end_src
- [ ] Add ~TransposingGrid~ container, which has child ~TransposingStackLayouter~, with 2 children, ~TransposingInputValuesGrid~ (perpendicular to TransposingInputAxis) and ~TransposingOutputValuesGrid~
- [ ] Test with TransposingGrid is empty! (rather the input/output gridlines have no children) *results should not change*
- [ ] Add (first only) TransposingInputValuesGrid, which has lines layed out using ExternalTicksColumn. Must use same layout as labels. ~todo-00~ continue design here.
...
*** ~Start work on dotted grid lines collecting requirements for StackingLayouter changes~
*** ~Add (reuse or improve existing) StackingLayouter~

** TODO-NEXT ~Change example and test directory structure around main.dart.  Move example/lib/src/main.dart to test/lib/src/test_main.dart; create standard and alternative layouts~
...
** IN-PROGRESS ~16. Figure out how to create and keep some basic layout templates - e.g. to pre-layout Y axis~ 4 days. Should start June 2, end June 5.
