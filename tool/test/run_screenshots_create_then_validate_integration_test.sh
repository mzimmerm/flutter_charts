...
for layout in auto_layout coded_layout; do
  ...
  if [[ $layout == coded_layout ]]; then
    ...
    # are copied to their coded_layout equivalents and edited to perform a test for coded_layout.
    # Reason for this: We want to keep only one core copy of the test app,
    # the test_main.dart and duplicate it (edited) as coded_layout_test_main.dart.
