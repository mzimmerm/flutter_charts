...
* TODO New in the current release
...
Below is an illustration of how the "iteratively auto layout" behaves when less and less horizontal space is available to display the chart. Chart automatically senses the X label overlap, and follows with rule-based iterative re-layout, to prevent labels running into each other.
